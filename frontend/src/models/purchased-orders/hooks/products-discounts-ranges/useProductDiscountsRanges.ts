...
import { fetchProductsDiscountRangeFromDB } from "./fetchs";
import type { AppDispatchRedux } from "../../../../store/store";
import { setError, clearError } from "../../../../store/slicer/errorSlicer";
import type { CartProduct } from "../../../../interfaces/products-cart";
...
const useDiscountRanges = (productId: number | undefined) => {
    ...
    const fetchDiscountRanges = async () => {
        ...
        try {
            const data =
                await fetchProductsDiscountRangeFromDB(dispatch, productId || 0);
            if (data) {
                setDiscountRanges(data);
            }
        } catch (err: unknown) {
            const msg = err instanceof Error ? { validation: err.message } : { validation: "Unknown error" };
            dispatch(setError({ key: "discountRanges", message: msg }));
        } finally {
    ...
    return {
        ...
        refetchDiscountRanges: fetchDiscountRanges,
    };
};
