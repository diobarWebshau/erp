...
import ImageUploader from "../../../../components/ui/general/uploaderImage/ImageUploader";
import base64ToFileOrNull from "../../../../scripts/convertBase64ToFile";

interface EditModalProps {
    onClose: Dispatch<SetStateAction<boolean>>
    onUpdate: (value: IPartialShippingOrder) => void,
    record: IPartialShippingOrder | null
}
...
const EditModal = (
    ...
    const [deliveryCost, setDeliveryCost] =
        useState<number | undefined>(
            Number(record?.delivery_cost) ?? undefined
        );
    const [client, setClient] =
        useState<IClient | undefined>(
            record
                ?.shipping_order_purchase_order_product
                ?.[0]
                ?.purchase_order_products
                ?.purchase_order
                ?.client as IClient ?? undefined
        );
    const [clientAddresses, setClientAddress] =
        useState<IClientAddress | undefined>(
            record
                ?.shipping_order_purchase_order_product
                ?.[0]
                ?.purchase_order_products
                ?.purchase_order
                ?.client_address as IClientAddress ?? undefined
        );
    const [purchasedOrderProductsSelected, setPurchasedOrderProductsSelected] =
        useState<IPurchasedOrderProduct[]>(
            record
                ...
                ?.map(
                    (item) =>
                        item.purchase_order_products) as
            IPurchasedOrderProduct[] ?? []
        );
    const [customValues, setCustomValues] =
        useState<Record<number, number>>(() => {
            ...
            return products.reduce((acc, item) => {
                const id = item?.purchase_order_products?.id;
                const qty = item?.qty;
                acc[id as number] = qty as number;
                return acc;
    ...
    const handleOnClickAdd = (e: MouseEvent<HTMLButtonElement>) => {
        ...
        if (carrier &&
            ...
            const evidence =
                record?.load_evidence as LoadEvidenceItem[]
            
            const imagesLoadEvidence: LoadEvidenceItem[] = images.map(
                (image) => ({
                    path: image, 
                    id: evidence?.find(
                        (item: LoadEvidenceItem) =>{
                            return item?.id === image.name
                        }
                    )?.id ?? ""
                })
            )
            ...
            const shipping: IPartialShippingOrder = {
                ...
                shipping_order_purchase_order_product: shippingProducts,
