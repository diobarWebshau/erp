...
interface Step3Props {
    ...
    onCreate: (data: IPartialPurchasedOrder) => Promise<boolean>;
}
...
const Step3 = ({
    ...
    const excludeIds = useMemo<number[]>(() => state.data?.purchase_order_products?.map(p => p.product_id as number) ?? [], [state.data?.purchase_order_products]);
    ...
    }, [dateCreation, dateDeliver, dispatch, onCreate, state.data, toggleConfirmModal]);
