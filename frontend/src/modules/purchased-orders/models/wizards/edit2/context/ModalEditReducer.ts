    ...
    (
        ...
        switch (action.type) {
            case ModalEditActionsTypes.SET_PURCHASE_ORDER: {
                const purchasedOrder: IPartialPurchasedOrder = action.payload;
                const purchasedOrderProducts = purchasedOrder.purchase_order_products || [];
                ...
                if (purchasedOrderProducts.length > 0) {
                    const newPurchasedOrderProducts = purchasedOrderProducts.map((p) => {
                        ...
                        if (rangeDiscount || clientDiscounts) {
            ...
            case ModalEditActionsTypes.ADD_PURCHASE_ORDER_PRODUCTS: {
                const products = draft.updated.purchase_order_products;
                const newProducts = action.payload as IPartialPurchasedOrderProduct[];

                if (!products || newProducts.length === 0) break;

                const client_discounts =
                    draft.updated.client?.product_discounts_client ?? [];
                ...
                const productsToAdd = newProducts.map((p) => {
                    ...
                    const finalPriceWithClientDiscount = discountClientApply
                        ? safeBasePrice *
                        (1 - (discountClientApply.discount_percentage ?? 0) / 100)
                        : safeBasePrice;
            ...
            case ModalEditActionsTypes.SET_PURCHASE_ORDER_PRODUCTS_QTY: {
                ...
                if (products.length > 0) {
                    ...
                    if (purchasedProduct) {
                        const product = purchasedProduct.product;
                        const qty = action.payload.qty;

                        const discountRanges = product?.product_discount_ranges ?? [];

                        const discountRangeApply = discountRanges.find((range) => {
                            const min = safeNumber(range.min_qty);
                            const max = safeNumber(range.max_qty);
                        ...
                        const discountClientApply = clientDiscounts.find(
                            (d) => d.product_id === action.payload.id
                        );

                        // --- Precio base
                        const basePrice = discountRangeApply
                            ? safeNumber(discountRangeApply.unit_price)
                            : safeNumber(purchasedProduct.original_price);

                        // --- Precio con descuento cliente
                        const finalPriceWithClientDiscount =
                            basePrice *
                            (1 - (safeNumber(discountClientApply?.discount_percentage) / 100));

                        purchasedProduct.qty = qty;

                        // --- Reglas finales
                        if (discountRangeApply || discountClientApply) {
                            purchasedProduct.recorded_price =
                                Number(safeNumber(finalPriceWithClientDiscount).toFixed(2));
                            purchasedProduct.was_price_edited_manually = false;
                        } else {
                            purchasedProduct.recorded_price =
                                Number(safeNumber(product?.sale_price).toFixed(2));
                            purchasedProduct.was_price_edited_manually = null;
                        }

                        // --- Fallback absoluto
                        if (
                            typeof purchasedProduct.recorded_price !== "number" ||
                            isNaN(purchasedProduct.recorded_price) ||
                            purchasedProduct.recorded_price <= 0
                        ) {
                            purchasedProduct.recorded_price =
                                Number(safeNumber(product?.sale_price).toFixed(2));
                            purchasedProduct.was_price_edited_manually = null;
