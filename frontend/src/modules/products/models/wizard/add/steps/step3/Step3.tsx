...
import { type ReactNode, useCallback, useMemo, useState, type Dispatch } from "react";
...
interface Step3Props {
    ...
    onCancel: () => void,
    onCreate: (item: IPartialItem) => Promise<boolean>
    onClose: () => void
}

const Step3 = ({ state, dispatch, onCancel, onCreate, onClose }: Step3Props) => {
    ...
    const handleOnClickNextStep = useCallback(async (): Promise<void> => {
        const ok: boolean = await onCreate(state.data);
        if (!ok) {
        ...
        const customMessage: ReactNode = (
            <span className={`nunito-bold ${StyleModule.customMessageConfirmModal}`}>
                El producto {` `} <span>{state.data.item?.name}</span> {` `} se ha agregado correctamente.
            </span>
        );
        setCustomMessageConfirmModal(customMessage);
        toggleShowConfirmModal();
    }, [onCreate, state.data, errorRedux, toggleShowConfirmModal]);
