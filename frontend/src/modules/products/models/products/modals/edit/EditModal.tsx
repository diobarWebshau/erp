...
const EditModal = ({
    ...
    const [proccessesAsigned, setProccessesAsigned] =
        useState<IPartialProductProcess[]>(record?.product_processes || []);
    const [discountsAsigned, setDiscountsAsigned] =
        useState<IPartialProductDiscountRange[]>(record?.product_discount_ranges || []);
    const [inputsAsigned, setInputsAsigned] =
        useState<IPartialProductInput[]>(record?.products_inputs || []);
    const [equivalence, setEquivalence] =
        useState<number | undefined>();
    const [inputSelected, setInputSelected] =
        useState<IInput | undefined>(undefined);
    const [name, setName] =
        useState<string | undefined>(record?.name);
    const [sku, setSku] =
        useState<string | undefined>(record?.sku);
    const [description, setDescription] =
        useState<string | undefined>(record?.description);
    const [type, setType] =
        useState<string | undefined>(record?.type);
    const [isActive, setIsActive] =
        useState<boolean | undefined>(record?.active);
    const [salePrice, setSalePrice] =
        useState<number | undefined>(Number(record?.sale_price));
    ...
    const [isHasProccesses, setIsHasProccesses] =
        useState<boolean | undefined>(
            record?.product_processes
                ? record.product_processes.length > 0
                : undefined
        );
    const [isHasInputs, setIsHasInputs] =
        useState<boolean | undefined>(
            record?.products_inputs
                ? record.products_inputs.length > 0
                : undefined
        );
    const [isHasDiscountRanges, setIsHasDiscountRanges] =
        useState<boolean | undefined>(
            record?.product_discount_ranges
                ? record.product_discount_ranges.length > 0
                : undefined
        );
                                    ...
                                    <DraggableList
                                        ...
                                        onItemsChange={(newItems) => {
                                            ...
                                            setProccessesAsigned(newItems.map(({ id, ...rest }) => ({
                                                ...rest,
                                                id: Number(id), // opcional si ya lo tienes
                                            })));
                                        }}
