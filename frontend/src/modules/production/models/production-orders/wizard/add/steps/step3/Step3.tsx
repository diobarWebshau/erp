...
import { Calendar, ChevronDown, ChevronLeft, ChevronRight, CircleCheck, CircleX, FileCheck2, Search } from "lucide-react";
...
const Step3 = () => {
    ...
    const handlerOnChangeSelectedProduct = (product: IProduct | null) => {
        if (product) {
    ...
    const handlerOnChangeSelectedLocation = (location: ILocation | null | undefined) => {
        if (location) {
            dispatch(update_production_order({
                location: location,
            }))
    ...
    const handlerOnChangeSelectedProductionLine = (productionLine: IProductionLine | null | undefined) => {
        if (productionLine) {
            dispatch(update_production_order({
    ...
    useWhatChanged([state.data.location, state.data.product], ['location', 'product']);
            ...
            <section className={StyleModule.bodySection}>
                ...
                <div className={StyleModule.containerTextArea}>
                    <StandardTextArea
                        ...
                        classNameLabel={StyleModule.labelTextArea}
                        classNameTextArea={`nunito-semibold ${StyleModule.textArea}`}
                        classNameError={StyleModule.errorTextArea}
                        classNameHelp={StyleModule.helpTextArea}
                        id="notes"
                        placeholder="Escribe aqui los comentarios"
                    />
                </div>
            </section>
