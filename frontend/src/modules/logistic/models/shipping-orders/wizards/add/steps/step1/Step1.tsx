...
const Step1 = ({ onClose }: IStep1) => {
    ...
    const columns: ColumnDef<IPurchasedOrder>[] = useMemo(() => [
        ...
        },
        {
            ...
            accessorFn: (row) => `${row.shipping_street}, ${row.shipping_street_number}, ${row.shipping_neighborhood}, ${row.shipping_city}, ${row.shipping_state}, ${row.shipping_zip_code}`,
            meta: {
                hidden: false,
                autoGenerated: true,
                type: "string",
    ...
    const conditionalRowSelection = useCallback((updater: RowSelectionState, rows: Row<IPurchasedOrder>[],): boolean => {
        if (selectedPurchasedOrder.length === 0) {
            if (rows.length === Object.keys(updater).length) {
                ...
                const client_address_id = rows[0].original.client_address_id;
                ...
                const isSameAddress = records.every(
                    (p: IPurchasedOrder) => {
                        const sameAddress = p.client_address_id === client_address_id;
                        return sameAddress;
                    }
                );
            ...
            const client_address_id = selectedPurchasedOrder[0].client_address_id;
            const isSameClient = records.every(
                (p: IPurchasedOrder) => {
                    const sameClient = p.client?.company_name === client;
                    return sameClient;
