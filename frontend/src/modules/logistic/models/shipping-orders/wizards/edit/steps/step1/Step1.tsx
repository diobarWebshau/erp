...
const Step1 = () => {
    ...
    const [selectedSopops, initialTableState] = useMemo(() => {
        const selectedSopops = state.draft?.shipping_order_purchase_order_product || [];
    ...
    const handleOnClickNextStep = useCallback(async () => {
        ...
        if (diffObject.added.length > 0) {
            ...
            dispatch(add_draft_shipping_order_purchased_order_products(added));
        }
        if (diffObject.deleted.length > 0) {
            const ids = new Set(diffObject.deleted.map(d => d.id));
            dispatch(remove_draft_shipping_order_purchased_order_products([...ids]));
        }
    ...
    const columns: ColumnDef<IPartialShippingOrderPurchasedOrderProduct>[] = useMemo(() => [
        ...
        },
        {
            ...
            cell: (row) => {
                const record = row.row.original;
                const stock = record.purchase_order_products?.stock_available?.stock || 0;
        ...
        },
        {
            ...
            cell: (row) => {
                const record = row.row.original;
                const finished = record.purchase_order_products?.production_order?.production_breakdown?.finished || 0;
                const order_qty = record.purchase_order_products?.production_order?.production_breakdown?.order_qty || 0;
                return (
                    <span className={StyleModule.containerProgressQtyProduction}>
                        {finished === order_qty
                            ? <Tag className={StyleModule.tagCompletedProduction} label="Completado" />
                            : (
                                <SingleProgressBarMantineCustom
                                    value={finished}
                                    total={order_qty}
                                />
                            )
        ...
        <div className={StyleModule.container}>
            ...
            <div className={StyleModule.content}>
                <GeneralTable
                    ...
                    getRowId={(row: IPartialShippingOrderPurchasedOrderProduct) => row.id?.toString() || ""}
