...
const SingleSelectSearchCheck = <T,>({
    ...
    const onClickAddItem = (item: T) => {
        if (disabled) return; // <--- BLOQUEA ACCIÓN
    ...
    return (
        <div className={`${classNameContainer ?? ""} ${style.container} ${disabled ? style.disabled : ""}`}>
            <div>
                <div className={`${classNameInputContainer ?? ""} ${style.inputContainer}`}>
                    <input
                        className={`${classNameInput ?? ""} ${style.input} ${disabled ? style.disabled : ""}`}
                        type="text"
                        value={search}
                    ...
                    <button
                        ref={refButton}
                        className={`${classNameButtonInput ?? ""} ${style.buttonIconInput} ${disabled ? style.disabled : ""}`}
                            ...
                            {selectedArray.map((item) => {
                                ...
                                return (
                                    <div
                                        ...
                                        tabIndex={0}
                                        onKeyDown={(e) => {
                                            if (!disabled && (e.key === "Enter" || e.key === " ")) onClickAddItem(item);
                                        }}
                                    >
