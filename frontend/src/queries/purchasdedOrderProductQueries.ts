...
const fetchPurchasedOrderProductsByClientAddressFromDB = async (
    dispatch: AppDispatchRedux,
    client_address_id: number | undefined
): Promise<IPurchasedOrderProduct[]> => {
    try {
        const response = await fetch(`${API_URL}/client-address/${client_address_id}`, {
            method: "GET",
            headers: { "Content-Type": "application/json" },
        });
        if (!response.ok) {
            ...
            dispatch(
                setError({
                    key: "PurchasedOrderProductsByClientAddress",
                    message: errorText
                })
            );
            return [];
        ...
        dispatch(
            clearError("PurchasedOrderProductsByClientAddress")
        );
        const data: IPurchasedOrderProduct[] = await response.json();
        return data;
    } catch (error: unknown) {
...
const revertProductionOrderOfProductOrderProductFromDB = async (
    id: number,
    dispatch: AppDispatchRedux
): Promise<IPurchasedOrderProduct[]> => {
    try {
        ...
        if (!response.ok) {
            ...
            dispatch(
                setError({
                    key: "revertProductionOrderOfPurchasedOrderProducts",
                    message: errorText
                })
            );
            return [];
        ...
        dispatch(
            clearError("revertProductionOrderOfPurchasedOrderProducts")
        );
        const data: IPurchasedOrderProduct[] = []
        return data;
    } catch (error: unknown) {
        throw error;
    }
...
const getPurchasedOrderProductsByPOFromDB = async (
    ...
    try {
        ...
        if (!response.ok) {
            ...
            dispatch(
                setError({
                    key: "PurchasedOrderProductsByPO",
                    message: errorText
                })
            );
            return [];
        ...
        dispatch(
            clearError("PurchasedOrderProductsByPO")
        );
        const data: IPurchasedOrderProduct[] = await response.json();
        return data;
    } catch (error: unknown) {
