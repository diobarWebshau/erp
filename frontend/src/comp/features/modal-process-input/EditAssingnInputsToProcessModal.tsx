...
import StyleModule from "./EditAssingnInputsToProcessModal.module.css"
import { useCallback, useMemo, useState } from "react";
import type { ColumnDef } from "@tanstack/react-table";
import { Plus, Trash2 } from "lucide-react";

interface IEditAssingnInputsToProcessModalProps {
    mode: "new" | "asingn",
    fetchLoadProcess: (query: string | number) => Promise<IPartialProcess[]>,
    validInputs?: IPartialProductInput[],
    value: IPartialProductProcess
    onClose: () => void,
    onClick: (values: IPartialProductProcess[]) => void
}

const EditAssingnInputsToProcessModal = ({
    onClose,
    onClick,
    validInputs,
    mode,
    fetchLoadProcess, value
}: IEditAssingnInputsToProcessModalProps) => {

    // ***************  States ***************

    const [nameProcess, setNameProcess] = useState<string | null>(value.process?.name ?? null);
    const [descriptionProcess, setDescriptionProcess] = useState<string | null>(value.process?.description ?? null);
    const [selectedProcessAux, setSelectedProcessAux] = useState<IPartialProcess | null>(value?.process ?? null);
    const [selectedProductInputProcess, setSelectedProductInputProcess] = useState<IPartialProductInputProcess[]>(value.product_input_process ?? []);
    ...
    const handleOnClickAddButton = useCallback(() => {
        ...
        console.log(selectedProductInputProcessWithProcess);
        onClick([selectedProductInputProcessWithProcess]);
        onClose();
    }, [
        mode, value, selectedProductInputProcess, onClick,
...
export default EditAssingnInputsToProcessModal;