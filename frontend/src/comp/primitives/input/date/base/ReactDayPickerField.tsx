...
const ReactDayPickerField = ({
    ...
    const [isValidDateValue, setIsValidDateValue] = useState(() =>
    ...
    const handleDayPickerSelect = (date: Date | undefined) => {
        if (!date) {
            ...
            onChange?.(undefined);
            setIsValidDateValue(false);
        } else {
            setSelectedDate(date);
            setInputValue(format(date, "MM/dd/yyyy"));
            onChange?.(date);
            setIsValidDateValue(true);
        }
        setIsDialogOpen(false);
    };
    ...
    useEffect(() => {
        setIsValidDateValue(value ? isValid(value) : false);
    }, [value]);
    ...
    return (
        <div className={`${stylesModules.container} ${classNameContainer ?? ""}`}>
            ...
            <div className={`${stylesModules.dateField}  ${classNameField ?? ""}`}>
                <input
                    className={`${stylesModules.dateInput} ${!isValidDateValue && stylesModules.dateFieldInvalid}  ${classNameInput ?? ""}`}
                    id="date-input"
                    type="text"
                    value={inputValue}
