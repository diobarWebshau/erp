...
import useDndSensors from "./dnd-kit/useDndSensors";
...
const TableBase = <T,>({
    ...
    const memoProcessedColumns = useMemo(() => {
        ...
        if (enableSortableRows && getSortField && setSortField) {
            parts.push({
                id: "drag",
                header: () => null,
                cell: () => <GripVertical size={20} color="var(--color-theme-primary-light)" />,
                enableSorting: false,
                enableHiding: false,
    ...
    */
    ...
    const [
        ...
        const containerTableClassNames =
            ` ${stylesModules.containerTable} ${classNameTableContainer}` +
            ` ${modeTable === "content" ? stylesModules.noScrollTable : stylesModules.scrollTable}
        ...
        extraComponents, footerComponents, classNameFooter, classNameExtraComponents,
        classNameGenericTableContainer, classNameTable, classNameTableBody,
        classNameTableContainer, classNameTableHeader, modeTable, enablePagination
    ]);
    ...
    const { items, onDragEnd } = useDndTableRows({
        rows: data,
        enableSortableRows: enableSortableRows,
        getSortField: getSortField,
        setSortField: setSortField,
        setOnReorderRows: (setOnReorderRows ? setOnReorderRows : () => { }),
        getId: getRowId
    });
