...
const TableBase = <T,>({
    ...
    const prepareEnumColumn = <T,>(columns: ColumnDef<T>[]): ColumnDef<T>[] => {
        return columns.map((col) => {
            if (col.meta?.type === "enum") {
                return {
                    ...col,
                    meta: {
                        ...col.meta,
                        enumOptions: col.meta.enumOptions?.map((option) => ({
                            value: option.value,
                            label: option.label
                        }))
                    }
        ...
        ...(enableRowSelection && {
            onRowSelectionChange: (updater: Updater<RowSelectionState>) => {
                const rowSelectionState = typeof updater === 'function'
                    ...
                    : updater;
                const condition = conditionalRowSelection
                    ? conditionalRowSelection(rowSelectionState, table.getRowModel().rows)
                    : true;
                if (condition) dispatch(set_row_selection(rowSelectionState))
                else console.log("No cumple con la condicion");
            }
        }),
    ...
    useEffect(() => {
        if (firstNotifyRef.current) { // <- evita disparo en el montaje
            firstNotifyRef.current = false;
            return;
        }
        const selected = tableRef.current.getSelectedRowModel().rows.map(r => r.original);
        onRowSelectionChange?.(selected);
    }, [state.rowSelectionState, onRowSelectionChange]);
            ...
            >
                ...
                >
                    {table?.getRowModel()?.rows?.length > 0 ? (
                        ...
                        <table
                            className={
                                `${stylesModules.table} ${stylesModules.tableWithoutRow} ${classNameTable} ` +
                                `${footerComponents ? stylesModules.tableWithoutFooter : ""}`
                            }
                        >
                            {
