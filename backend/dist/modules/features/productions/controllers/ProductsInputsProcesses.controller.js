...
import { ProductInputProcessModel, ProductInputModel, ProductProcessModel } from "../../../associations.js";
...
class ProductInputProcessController {
    static getAll = async (req, res, next) => {
        try {
            ...
            const relationships = response.map((rip) => rip.toJSON());
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (Object.keys(update_values).length === 0) {
                await t.rollback();
                res.status(400).json({
                    validation: "No validated properties were found for updating the Product-Input-Process relationship"
                });
                return;
            }
            ...
            const next_product_id = (update_values.product_id ?? relationship.product_id);
            const next_product_input_id = (update_values.product_input_id ?? relationship.product_input_id);
            const next_product_process_id = (update_values.product_process_id ?? relationship.product_process_id);
            const next_qty = (update_values.qty != null ? Number(update_values.qty) : Number(relationship.qty));
            ...
            // 6) Actualizar
            const response = await ProductInputProcessModel.update(update_values, { where: { id }, individualHooks: true, transaction: t });
            if (!(response[0] > 0)) {
                await t.rollback();
                res.status(200).json({
                    validation: "No changes were made to the Product-Input-Process relationship"
                });
                return;
            }
            await t.commit();
    ...
    static deleteById = async (req, res, next) => {
        ...
        try {
            ...
            if (!(response > 0)) {
                res.status(200).json({
                    validation: "Product-Input-Process relationship not found for delete"
                });
                return;
            }
            res.status(200).json({
