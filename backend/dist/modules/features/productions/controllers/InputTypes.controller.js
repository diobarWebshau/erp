...
class InputTypesController {
    ...
    static create = async (req, res, next) => {
        ...
        try {
            ...
            if (validateName) {
                res.status(200).json({
                    validation: "The name is already currently in use by a input type"
                });
                return;
            }
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (Object.keys(update_values).length === 0) {
                res.status(400).json({
                    validation: "No validated  inputs properties were found for updating"
                });
                return;
            }
            ...
            const response = await InputTypeModel.update(update_values, { where: { id: id }, individualHooks: true });
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation: "The input type could not be created"
                });
    ...
    static getInputsWithThisType = async (req, res, next) => {
        const { id } = req.params;
        try {
            const validateInputType = await InputTypeModel.findOne({
                where: { id: id }
            ...
            if (!(response.length > 0)) {
                res.status(200).json({
                    validation: "Inputs no found for this input type"
                });
                return;
            }
