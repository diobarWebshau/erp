...
import { ProductModel, InputModel, ProductInputModel } from "../../../associations.js";
...
class ProductInputController {
    ...
    static create = async (req, res, next) => {
        ...
        try {
            ...
            if (!response) {
                res.status(400).json({
                    validation: "The product-input relationship could not be created"
                });
                return;
            }
            res.status(201).json({
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!validateRelationship) {
                res.status(404).json({
                    validation: "Product-input relationship no found for update"
                });
                return;
            }
            ...
            const validateDuplicate = await ProductInputModel.findOne({
                where: {
                    [Op.and]: [
                        ...
                        {
                            input_id: update_values.input_id
            ...
            if (validateDuplicate) {
                res.status(409).json({
                    validation: "Product-Input relationshio already exists"
                });
                return;
            }
            const response = await ProductInputModel.update(update_values, { where: { id: id }, individualHooks: true });
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation: "No changes were made to the process assignment to the product"
                });
                return;
            }
            res.status(200).json({
                message: "Product-input relationship updated succefally"
            });
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    static deleteById = async (req, res, next) => {
        ...
        try {
            ...
            if (!(response > 0)) {
                res.status(200).json({
                    validation: "Products-input relationship no found for delete"
                });
                return;
            }
            res.status(200).json({
                message: "Product-Input relationship deleted successfully"
            });
        }
        catch (error) {
            if (error instanceof Error) {
