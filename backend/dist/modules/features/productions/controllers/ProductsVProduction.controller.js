...
import { ProductModel, ProcessModel, InputModel, ProductDiscountRangeModel, ProductInputModel, ProductProcessModel, } from "../../../associations.js";
...
class ProductVProductionController extends ProductsControllers.ProductController {
    static getInputsOfProduct = async (req, res, next) => {
        const { id } = req.params;
        try {
            ...
            // }));
            const inputsOfProducts = response.map(ip => ip.toJSON());
            const inputsOfProductsDetails = await Promise.all(inputsOfProducts.map(async (product) => {
                const imagePath = product.photo;
                const imageBase64 = await ImageHandler.convertToBase64(imagePath);
                product.photo = imageBase64 || null;
                const products_inputs = product.products_inputs;
                const product_inputs_details = await Promise.all(products_inputs.map(async (product_input) => {
                    if (Array.isArray(product_input)) {
                        const inputsDetails = await Promise.all(product_input.map(async (input) => {
                            const imgPath = input.photo;
                            const imgBase64 = await ImageHandler.convertToBase64(imgPath);
                            input.photo = imgBase64;
                            return input;
    ...
    static addInputToProduct = async (req, res, next) => {
    ...
    static deleteInput = async (req, res, next) => {
        const { id, input_id } = req.params;
        try {
            const validateProduct = await ProductModel.findOne({ where: { id: id } });
            if (!validateProduct) {
    ...
    static EditEquivalenceInProductInput = async (req, res, next) => {
        const { id, input_id } = req.params;
        const { equivalence } = req.body;
        try {
            const validateProduct = await ProductModel.findOne({ where: { id: id } });
            ...
            const response = await ProductInputModel.update({ equivalence: equivalence }, {
                where: {
                    [Op.and]: [
                        { product_id: id },
                        { input_id: input_id }
    ...
    static getProcessesOfProduct = async (req, res, next) => {
        const { id } = req.params;
        try {
            const validateProduct = await ProductModel.findOne({ where: { id: id } });
            if (!validateProduct) {
    ...
    static assignProcessToProduct = async (req, res, next) => {
    ...
    static deleteProcess = async (req, res, next) => {
        const { id, process_id } = req.params;
        try {
            ...
            const response = await ProductProcessModel.destroy({
                where: {
                    [Op.and]: [
                        { product_id: id },
                        { process_id: process_id }
            ...
            if (!(response > 0)) {
                res.status(200).json({
                    validation: "Process not found in product to be deleted"
                });
                return;
            }
            res.status(200).json({
    ...
    static createCompleteProduct = async (req, res, next) => {
        ...
        try {
            const [validateName, validateSku] = await Promise.all([
            ...
            if (validateSku) {
                ...
                await ImageHandler.removeImageIfExists(photo);
                res.status(409).json({
                    validation: `The sku is already currently `
                        + `in use by a product`
                });
                return;
            }
            ...
            if (product_processes) {
                const processesObject = JSON.parse(product_processes.toString());
                if (processesObject.length > 0) {
                    const processesIds = processesObject.map((p) => p.process?.id)
                        .filter((id) => id !== undefined);
                    const responseValidationProcesses = await ProductProcessModel.findAll({
                        where: {
                    ...
                    if (responseValidationProcesses.length > 0) {
                        ...
                        await ImageHandler.removeImageIfExists(photo);
                        res.status(400).json({
                            validation: `Some processes are already assigned `
                                + `to this product`
                        });
                        return;
                    }
                    const productProcess = processesObject
                        .map((p) => ({
                        product_id: product.id,
                        process_id: p.process?.id,
                        sort_order: p.sort_order
                    }))
                        .filter((p) => p.process_id !== undefined);
                    const responseProcesses = await ProductProcessModel.bulkCreate(productProcess, {
                        individualHooks: true,
                        transaction: transaction
                    });
                    if (!responseProcesses) {
                        ...
                        await ImageHandler.removeImageIfExists(photo);
                        res.status(400).json({
                            validation: "The processes could not be created"
                        });
                        return;
                    }
                }
            ...
            if (products_inputs) {
                const productsInputsObject = JSON.parse(products_inputs.toString());
                if (productsInputsObject.length > 0) {
                    const inputsIds = productsInputsObject
                        .map((i) => i.inputs?.id)
                        .filter((id) => id !== undefined);
                    const responseValidationInputs = await ProductInputModel.findAll({
                        where: {
                            product_id: product.id,
                            input_id: inputsIds
                        }
                    ...
                    if (responseValidationInputs.length > 0) {
                        ...
                        await ImageHandler.removeImageIfExists(photo);
                        res.status(400).json({
                            validation: "Some inputs are already assigned to this product"
                        });
                        return;
                    }
                    ...
                    const responseInputs = await ProductInputModel.bulkCreate(productInputsFiltered, {
                        individualHooks: true,
                        transaction: transaction
                    });
                    if (!responseInputs) {
                        ...
                        res.status(400).json({
                            validation: "The inputs could not be created"
                        });
                        return;
                    }
                }
            ...
            if (product_discount_ranges) {
                const productDiscountRangesObject = JSON.parse(product_discount_ranges.toString());
                if (productDiscountRangesObject.length > 0) {
                    const productDiscountRanges = productDiscountRangesObject
                        .map((pdr) => ({
                        product_id: product.id,
                    ...
                    const validateDiscountRanges = await ProductDiscountRangeModel.findAll({
                        where: {
                            product_id: product.id,
                    ...
                    if (validateDiscountRanges.length > 0) {
                        ...
                        await ImageHandler.removeImageIfExists(photo);
                        res.status(400).json({
                            validation: `Some discount ranges are already`
                                + ` assigned to this product`
                        });
                        return;
                    }
                    const responseDiscountRanges = await ProductDiscountRangeModel.bulkCreate(productDiscountRanges, {
                        transaction: transaction,
                        individualHooks: true
                    });
                    if (!responseDiscountRanges) {
                        ...
                        await ImageHandler.removeImageIfExists(photo);
                        res.status(400).json({
                            validation: "The discount ranges could not be created"
                        });
                        return;
                    }
                }
    ...
    static updateCompleteProduct = async (req, res, next) => {
        let urlImageOld = "";
        ...
        try {
            ...
            const update_values = collectorUpdateFields(editableFields, productBody);
            ...
            if (Object.keys(update_values).length > 0) {
                if (update_values?.name) {
                    ...
                    if (validateName) {
                        ...
                        if (productBody?.photo) {
                            await ImageHandler
                                .removeImageIfExists(productBody.photo);
                        }
                        res.status(200).json({
                            validation: `The name is already currently in `
                                + ` use by a product`
                        });
                        return;
                    }
                ...
                if (update_values?.sku) {
                    const validateSku = await ProductModel.findOne({
                        where: {
                            [Op.and]: [
                    ...
                    if (validateSku) {
                        ...
                        res.status(200).json({
                            validation: `The sku is already currently in use`
                                + ` by a product`
                        });
                        return;
                    }
                ...
                const responseProduct = await ProductModel.update(update_values, {
                    where: { id },
                    transaction: transaction
                });
                if (!responseProduct) {
                    ...
                    res.status(400).json({
                        validation: "The product could not be updated"
                    });
                    return;
                }
            }
            if (productBody.product_processes_updated) {
                const productProcessesObject = JSON.parse(productBody
                ...
                if (flagProccessUpdate) {
                    ...
                    const adds = productProcessesObject.added;
                    const deletes = productProcessesObject.deleted;
                    const modifies = productProcessesObject.modified;
                    if (deletes.length > 0) {
                        console.log("Inicio de eliminacion de procesos");
                        const deleteIds = deletes.map(p => p.process_id);
                        // console.log(deletes)
                        ...
                        if (validateProcess.length !== deleteIds.length) {
                            ...
                            res.status(404).json({
                                validation: `Some of the assigned processes do`
                                    + ` not exist`,
                            });
                            return;
                        }
                        // console.log(deleteIds);
                        // console.log(id);
                        const responseDelete = await ProductProcessModel.destroy({
                            where: {
                                product_id: id,
                                process_id: deleteIds,
                            },
                    ...
                    if (adds.length > 0) {
                        ...
                        const addIds = adds.map(p => p.product_id);
                        const alreadyAssigned = await ProductProcessModel.findAll({
                            where: {
                                product_id: id,
                                process_id: addIds,
                            },
                        ...
                        if (alreadyAssigned.length > 0) {
                            ...
                            res.status(409).json({
                                validation: `Some processes have already been assigned `
                                    + `to the product`,
                            });
                            return;
                        }
                        const assignments = adds.map((e) => ({
                            product_id: Number(id),
                        ...
                        // console.log("Entro");
                        const responseAdd = await ProductProcessModel
                        ...
                        if (!responseAdd) {
                            ...
                            res.status(400).json({
                                validation: `The processes could not be `
                    ...
                    if (modifies.length > 0) {
                        const modifyIds = modifies.filter(p => p.id !== undefined)
                            .map(p => p.id);
                        const validateProductProcess = await ProductProcessModel.findAll({
                            where: { id: modifyIds },
                            transaction,
                            lock: transaction.LOCK.SHARE,
                        });
                        ...
                        for (const p of modifies) {
                            const productProcess = p.id;
                            const { id: _, ...rest } = p;
                            console.log(productProcess);
                            console.log(rest);
                            const result = await ProductProcessModel
                                .update(rest, {
            ...
            if (productBody.product_discount_ranges_updated) {
                const productDiscountRangesObject = JSON.parse(productBody
                    .product_discount_ranges_updated
                    .toString());
                const flagDiscountRangeUpdate = [
                ...
                if (flagDiscountRangeUpdate) {
                    ...
                    const adds = productDiscountRangesObject.added;
                    const deletes = productDiscountRangesObject.deleted;
                    const modifies = productDiscountRangesObject.modified;
                    if (modifies.length > 0) {
                        console.log("Inicio de modificacion de rangos de descuento");
                        const modifiesFiltered = modifies.filter(p => p.id !== undefined);
                        const modifyIds = modifiesFiltered.map(p => p.id);
                        const validateProductDiscountRange = await ProductDiscountRangeModel.findAll({
                            where: { id: modifyIds },
                            transaction,
                            lock: transaction.LOCK.SHARE,
                        });
                        if (validateProductDiscountRange.length
                            !== modifyIds.length) {
                            await transaction.rollback();
                            if (productBody?.photo) {
                                await ImageHandler
                            ...
                            res.status(404).json({
                                validation: `Some of the assigned discount `
                                    + `ranges do not exist`,
                            });
                            return;
                        }
                        const conflictingDiscountRanges = await ProductDiscountRangeModel.findAll({
                            where: {
                                product_id: id,
                        ...
                        if (conflictingDiscountRanges.length > 0) {
                            ...
                            res.status(400).json({
                                validation: `Some of the discount ranges you are trying`
                        ...
                        if (!allUpdated) {
                            ...
                            res.status(400).json({
                                validation: `The discount ranges could not be `
                                    + `modified for the product`
                            });
                            return;
                        }
                    ...
                    if (deletes.length > 0) {
                        ...
                        const deletedCount = await ProductDiscountRangeModel.destroy({
                            where: { id: deleteIds },
                            transaction,
                        });
                    ...
                    if (adds.length > 0) {
                        ...
                        if (validateRangeDiscount.length > 0) {
                            const existingRanges = validateRangeDiscount.map(rd => rd.toJSON());
                            const hasConflict = adds.some(newRange => existingRanges.some(existing => {
                                const isOverlapping = newRange.min_qty < existing.max_qty &&
                                    newRange.max_qty > existing.min_qty;
                                const isExactDuplicate = newRange.min_qty === existing.min_qty &&
                            ...
                            if (hasConflict) {
                                ...
                                res.status(200).json({
                                    validation: `A discount range overlaps or duplicates ` +
                            ...
                            const responseProductDiscountRange = await ProductDiscountRangeModel.bulkCreate(productDiscountRange, {
                                transaction,
                            });
            ...
            if (productBody.products_inputs_updated) {
                const productsInputsObject = JSON.parse(productBody
                ...
                if (flagProductsInputsUpdate) {
                    ...
                    const adds = productsInputsObject.added;
                    const deletes = productsInputsObject.deleted;
                    const modifies = productsInputsObject.modified;
                    if (modifies.length > 0) {
                        console.log("Inicio de modificacion de insumos");
                        const modifiesFiltered = modifies.filter(p => p.id !== undefined);
                        const modifyIds = modifiesFiltered
                            .map(m => m.id);
                        const existingInputs = await ProductInputModel.findAll({
                            where: { id: modifyIds },
                            transaction,
                            lock: transaction.LOCK.SHARE,
                        });
                        if (existingInputs.length !== modifyIds.length) {
                            ...
                            res.status(404).json({
                                validation: `Some inputs you are trying`
                                    + ` to modify do not exist`,
                            });
                            return;
                        }
                        const conflictingInputs = await ProductInputModel.findAll({
                            where: {
                                product_id: id,
                        ...
                        if (conflictingInputs.length > 0) {
                            ...
                            res.status(400).json({
                                validation: `Some of the inputs you are trying`
                        ...
                        if (!allUpdated) {
                            ...
                            res.status(400).json({
                                validation: `The inputs could not be modified for`
                                    + ` the product`,
                            });
                            return;
                        }
                    ...
                    if (deletes.length > 0) {
                        ...
                        const deletesFiltered = deletes.filter(p => p.id !== undefined);
                        const deleteIds = deletesFiltered.map(p => p.id);
                        const validateProductInput = await ProductInputModel.findAll({
                            where: { id: deleteIds },
                            transaction,
                            lock: transaction.LOCK.SHARE,
                        });
                        if (validateProductInput.length !== deleteIds.length) {
                            ...
                            res.status(404).json({
                                validation: `Some of the assigned inputs do`
                                    + ` not exist`,
                            });
                            return;
                        }
                        const deletedCount = await ProductInputModel.destroy({
                            where: { id: deleteIds },
                            transaction,
                        });
                        const allDeleted = deletedCount === deleteIds.length;
                        if (!allDeleted) {
                            ...
                            res.status(400).json({
                                validation: `The inputs could not be deleted for`
                                    + ` the product`,
                            });
                            return;
                        }
                    ...
                    if (adds.length > 0) {
                        ...
                        const addIds = adds.map(p => p.input_id);
                        const validInputs = await InputModel.findAll({
                            where: { id: addIds },
                            transaction,
                            lock: transaction.LOCK.SHARE,
                        });
                        ...
                        const alreadyAssigned = await ProductInputModel.findAll({
                            where: {
                                product_id: id,
                                input_id: addIds,
                            },
                        ...
                        if (alreadyAssigned.length > 0) {
                            ...
                            res.status(400).json({
                                validation: `Some of the assigned inputs are `
                                    + ` already assigned to the product`,
                            });
                            return;
                        }
                        const add_values = adds.map((i) => ({
                            product_id: Number(id),
                            input_id: i.input_id,
                            equivalence: i.equivalence
                        }));
                        const responseProductInput = await ProductInputModel.bulkCreate(add_values, {
                            transaction,
                        });
                        if (!responseProductInput) {
                            ...
                            res.status(400).json({
                                validation: `The inputs could not be `
                                    + `added to the product`
                            });
                            return;
                        }
            ...
            if (isSuccessFully && IsupdateImage &&
                ...
                await ImageHandler
                    .removeImageIfExists(urlImageOld);
            }
        }
    };
    ...
    static getProductDetails = async (req, res, next) => {
        ...
        try {
            ...
            const formattedResponse = await formatImagesDeepRecursive([response], ["photo", "path"]);
