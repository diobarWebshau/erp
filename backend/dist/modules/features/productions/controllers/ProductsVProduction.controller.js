...
class ProductVProductionController extends ProductsControllers.ProductController {
    ...
    static createCompleteProduct = async (req, res, next) => {
        const { name, description, type, presentation, is_draft, production_cost, sku, active, sale_price, custom_id, photo, product_processes, products_inputs, product_discount_ranges, barcode, storage_conditions, unit_of_measure } = req.body;
        ...
        try {
            ...
            if (products_inputs) {
                const productsInputsObject = products_inputs;
                if (productsInputsObject.length > 0) {
                    ...
                    const inputsIds = productsInputsObject.map((i) => i.inputs?.id).filter((id) => id !== undefined);
                    ...
                    const productInputsFiltered = productInputs.filter((i) => i.input_id !== null);
            ...
            if (product_processes) {
                const productProcessesObject = product_processes;
                if (productProcessesObject.length > 0) {
                    ...
                    if (processesAssign.length) {
                        ...
                        productProcess.push(...processesAssign.map(p => ({
                            product_id: product.id,
                    ...
                    // --------------------------------
                    const responseProductProcess = await ProductProcessModel.bulkCreate(productProcess.map(p => ({
                        process_id: p.process_id,
            ...
            if (product_discount_ranges) {
                const productDiscountRangesObject = product_discount_ranges;
                if (productDiscountRangesObject.length > 0) {
                    const productDiscountRanges = productDiscountRangesObject.map((pdr) => ({
                        product_id: product.id,
                    ...
                    const validateDiscountRanges = await ProductDiscountRangeModel.findAll({
                        where: {
                            product_id: product.id,
                            unit_price: { [Op.in]: productDiscountRanges.map((pdr) => pdr.unit_price) },
                            min_qty: { [Op.in]: productDiscountRanges.map((pdr) => pdr.min_qty) },
                            max_qty: { [Op.in]: productDiscountRanges.map((pdr) => pdr.max_qty) }
                        }
                    });
    ...
    static updateCompleteProduct = async (req, res, next) => {
        ...
        try {
            ...
            if (productBody.product_processes_updated) {
                const productProcessesObject = productBody.product_processes_updated;
                ...
                if (flagProccessUpdate) {
                    ...
                    if (modifies.length > 0) {
                        ...
                        const modifyIds = modifies.filter(p => p.id !== undefined).map(p => p.id);
                        ...
                        for (const productProcessAux of modifies) {
                            const productProcessId = productProcessAux.id;
                            const { id: _, product_input_process_updated, ...rest } = productProcessAux;
                            const update_values = collectorUpdateFields(ProductProcessModel.getEditableFields(), rest);
                            // ? Si existen propiedades directas del modelo, las actualizamos
                            if (Object.keys(update_values).length) {
                                const [resultUpdateProductProcess] = await ProductProcessModel.update(update_values, {
                            ...
                            if (product_input_process_updated) {
                                ...
                                if (flagProccessInputProcessUpdate) {
                                    const adds = product_input_process_updated.added ?? [];
                                    const deletes = product_input_process_updated.deleted ?? [];
                                    const modifies = product_input_process_updated.modified ?? [];
                                    ...
                                    if (deletes.length) {
                                        ...
                                        const deletesIdFilterd = deletes.filter(d => d.id !== undefined).map((dt) => dt.id) ?? [];
                                    ...
                                    if (adds.length) {
                                        ...
                                        const newProductInputProcess = adds.map((a) => ({
                                            ...a,
                                            qty: a.qty,
                                            product_id: product.id,
                                    ...
                                    if (modifies.length) {
                                        ...
                                        // ? Validamos que existan los registros que se eliminaran
                                        const validationExists = await ProductInputProcessModel.findAll({
                                            where: { id: { [Op.in]: modifiedIdFilterd.map(m => m.id) } },
                                            transaction
                                        });
                                        ...
                                        for (const productInputProcessAux of modifiedIdFilterd) {
                                            const productInputProcessId = productInputProcessAux.id;
                                            const { id: _, ...rest } = productInputProcessAux;
                                            const updateValuesProductInputProcess = collectorUpdateFields(ProductInputProcessModel.getEditableFields(), rest);
                                            // ? Si existen propiedades directas del modelo, las actualizamos
                                            if (Object.keys(updateValuesProductInputProcess).length) {
                                                ...
                                                const [resultUpdateProductProcess] = await ProductInputProcessModel.update(updateValuesProductInputProcess, {
            ...
            if (productBody.product_discount_ranges_updated) {
                const productDiscountRangesObject = productBody.product_discount_ranges_updated;
                    ...
                    if (adds.length > 0) {
                        ...
                        if (validateRangeDiscount.length) {
                            ...
                            const hasConflict = adds.some((newRange) => {
                                ...
                                if (newRange.min_qty == null || newRange.max_qty == null)
                                    return false;
                                const newMin = Number(newRange.min_qty);
                                const newMax = Number(newRange.max_qty);
                                return existingRanges.some((existing) => {
                                    if (existing.min_qty == null || existing.max_qty == null)
                                        return false;
                                    const exMin = Number(existing.min_qty);
                                    const exMax = Number(existing.max_qty);
                                    const isOverlapping = newMin < exMax && newMax > exMin;
                                    const isExactDuplicate = newMin === exMin && newMax === exMax;
            ...
            if (productBody.products_inputs_updated) {
                const productsInputsObject = productBody.products_inputs_updated;
