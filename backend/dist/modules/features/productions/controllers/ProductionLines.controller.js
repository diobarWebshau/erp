...
import { ProductionLineModel, ProductModel, ProductionLineProductModel, LocationsProductionLinesModel, LocationModel } from "../../../associations.js";
...
class ProductionLinesController {
    ...
    static getProductionLineDetails = async (req, res, next) => {
        const { id } = req.params;
        try {
                ...
                include: [
                    {
                        ...
                        attributes: ProductionLineProductModel
                            .getAllFields(),
                        include: [
                            {
                                model: ProductModel,
    ...
    static getByName = async (req, res, next) => {
        ...
        try {
            const response = await ProductionLineModel.findOne({ where: { name: name } });
            if (!response) {
                res.status(200).json({ validation: "Produnction line no found" });
                return;
            }
    ...
    static createComplete = async (req, res, next) => {
        ...
        try {
            ...
            if (location_production_line) {
                ...
                const responseLocationProductionLine = await LocationsProductionLinesModel.create(locationProductionLineCreate, { transaction });
                if (!responseLocationProductionLine) {
                    await transaction.rollback();
                    res.status(400).json({
                        validation: `The assigned production line to`
            ...
            if (production_lines_products.length > 0) {
                const productsIds = production_lines_products.map((p) => p.products?.id);
                const responseValidationProducts = await ProductModel.findAll({
                    where: {
                        id: productsIds
                    }
                });
                if (responseValidationProducts.length
                    ...
                    await transaction.rollback();
                    res.status(400).json({
                        validation: "Some products are not valid"
                    });
                    return;
                }
                const producctionLineProductsCreate = production_lines_products.map((p) => {
                    return {
                        production_line_id: productionLine.id,
                        product_id: p.products?.id
                    };
                });
                const responseProductionLineProducts = await ProductionLineProductModel
                    .bulkCreate(producctionLineProductsCreate, { transaction });
                if (responseProductionLineProducts.length
                    !== producctionLineProductsCreate.length) {
                    await transaction.rollback();
                    res.status(400).json({
                        validation: "The production line products could not be created"
                    });
                    return;
                }
            }
    ...
    static updateComplete = async (req, res, next) => {
        ...
        try {
            const validateProductionLine = await ProductionLineModel
                .findOne({ where: { id: id } });
            ...
            const editableFields = ProductionLineModel.getEditableFields();
            ...
            if (Object.keys(update_values).length > 0) {
                ...
                const responseProductionLine = await ProductionLineModel.update(update_values, {
                    where: { id },
                ...
                if (!responseProductionLine) {
                    await transaction.rollback();
                    res.status(400).json({
                        validation: "The production line could not be updated"
                    });
                    return;
                }
            }
            if (Object.keys(completeBody?.location_production_line).length > 0) {
                ...
                if (Number(summary.internal_production) > 0) {
                    await transaction.rollback();
                    res.status(400).json({
                        validation: `You cannot change the location of` +
                ...
                const responseDelete = await LocationsProductionLinesModel.destroy({
                ...
                if (!responseDelete) {
                    await transaction.rollback();
                    res.status(400).json({
                        validation: `The production line location`
                ...
                const responseCreate = await LocationsProductionLinesModel.create(locationProductionLineCreate, { transaction });
                if (!responseCreate) {
                    await transaction.rollback();
                    res.status(400).json({
                        validation: `The production line location`
            ...
            if (completeBody?.production_lines_products_updated) {
                ...
                if (flagProductsInputsUpdate) {
                    const adds = plpManager.added;
                    const deletes = plpManager.deleted;
                    if (deletes.length > 0) {
                        ...
                        if (existingProducts.length !== deletesFiltered.length) {
                            await transaction.rollback();
                            res.status(404).json({
                                validation: `Some products you are trying`
                                    + ` to delete do not exist`,
                            });
                            return;
                        }
                        const responseDelete = await ProductionLineProductModel.destroy({
                            where: { id: { [Op.in]: deletesFiltered.map(d => d.id) } },
                            transaction
                        });
                        if (!responseDelete) {
                            await transaction.rollback();
                            res.status(400).json({
                                validation: `The production line products`
                                    + ` could not be deleted`
                            });
                            return;
                        }
                    ...
                    if (adds.length > 0) {
                        const addsFiltered = adds.filter(p => p.product_id !== undefined);
                        const addIds = addsFiltered
                            .map(m => m.product_id);
                        ...
                        const conflictingProducts = await ProductionLineProductModel.findAll({
                            where: {
                                [Op.and]: [
                        ...
                        const newAdds = addsFiltered.map(a => ({
                            production_line_id: Number(id),
                            product_id: a.product_id
                        }));
                        const responseAdd = await ProductionLineProductModel.bulkCreate(newAdds, { transaction });
                        if (!responseAdd) {
                            await transaction.rollback();
                            res.status(400).json({
                                validation: `The production line products`
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            const validateProductionLine = await ProductionLineModel.findOne({ where: { id: id } });
            if (!validateProductionLine) {
                res.status(200).json({
                    validation: "Production line not found"
                });
            ...
            if (update_values?.name) {
                ...
                if (validateName) {
                    res.status(200).json({
                        validation: "The name is already used in a production line"
                    });
                    return;
                }
            }
            const response = await ProductionLineModel.update(update_values, { where: { id: id }, individualHooks: true });
            if (!(response[0] > 0)) {
                res.status(200).json({
    ...
    static delete = async (req, res, next) => {
        ...
        try {
            ...
            if (Number(summary.internal_production) > 0) {
                res.status(400).json({
                    validation: "You cannot delete this production line " +
            ...
            const response = await ProductionLineModel.destroy({ where: { id: id }, individualHooks: true });
            if (!(response > 0)) {
                res.status(200).json({
    ...
    static getProductsOnProductionLine = async (req, res, next) => {
        const { id } = req.params;
        try {
            ...
            if (!response) {
                res.status(200).json({
                    validation: "No products found on the production line"
                });
                return;
            }
    ...
    static addProduct = async (req, res, next) => {
        ...
        try {
            ...
            if (validation) {
                res.status(200).json({
                    validation: "The product currently exists at the indicated production line."
                });
                return;
            }
            ...
            if (!response) {
                res.status(200).json({
                    alidation: "The product could not be added to the production line"
                });
                return;
            }
            res.status(200).json({
                validation: "Product added to production line successfully"
            });
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    static deleteProduct = async (req, res, next) => {
        ...
        try {
            ...
            if (!validatelocationProductionLine) {
                res.status(200).json({
                    validation: "Product not found in the production line to delete"
                });
                return;
            }
            ...
            res.status(200).json({
                message: "Product has been successfully removed from the production line"
            });
        }
        catch (error) {
            if (error instanceof Error) {
