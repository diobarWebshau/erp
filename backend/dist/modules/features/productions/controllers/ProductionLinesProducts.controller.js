import { ProductModel, ProductionLineModel, ProductionLineProductModel } from "../../../associations.js";
import collectorUpdateFields from "../../../../scripts/collectorUpdateField.js";
import { Op } from "sequelize";
class ProductionLineProductController {
    static getAll = async (req, res, next) => {
        try {
            const response = await ProductionLineProductModel.findAll();
            if (!(response.length > 0)) {
                res.status(200).json({
                    validation: "Production-Lines-Products "
    ...
    static getById = async (req, res, next) => {
        const { id } = req.params;
        try {
            const response = await ProductionLineProductModel.findByPk(id);
            if (!response) {
                res.status(200).json({
                    validation: "Production-Lines-Products relationship no found"
                });
                return;
            }
    ...
    static create = async (req, res, next) => {
        ...
        try {
            const [validateProductionLine, validateProduct] = await Promise.all([
            ...
            const validationDuplicate = await ProductionLineProductModel.findOne({
                where: {
                    [Op.and]: [
            ...
            if (validationDuplicate) {
                res.status(200).json({
                    validation: "Production-Lines-Products relationship"
                        + " already exists"
                });
                return;
            }
            ...
            if (!response) {
                res.status(400).json({
                    validation: "The product assigned to the production line"
                        + " could not be created"
                });
                return;
            }
            res.status(201).json({
                message: "Production-Lines-Products relationship created"
                    + " successfully"
            });
        }
        catch (error) {
    ...
    static update = async (req, res, next) => {
        ...
        const body = req.body;
        try {
            const validateRelationship = await ProductionLineProductModel.findOne({
                where: { id: id }
            });
            if (!validateRelationship) {
                res.status(200).json({
                    validation: "Production-Lines-Products no found for update"
                });
                return;
            }
            const relationship = validateRelationship?.toJSON();
            const editableFields = ProductionLineProductModel.getEditableFields();
            ...
            const validateDuplicate = await ProductionLineProductModel.findOne({
                where: {
                    [Op.and]: [
                        {
            ...
            if (validateDuplicate) {
                res.status(409).json({
                    validation: "The assigned product to the production line"
                        + " already exists"
                });
                return;
            }
            const response = await ProductionLineProductModel.update(update_values, { where: { id: id }, individualHooks: true });
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation: "ProductionLine-product relationship"
                        + " no found or no changes were made"
                });
                return;
            }
            res.status(200).json({
                message: "ProductionLine-product relationship "
                    + "updated succefally"
            });
        }
        catch (error) {
    ...
    static deleteById = async (req, res, next) => {
        ...
        try {
            ...
            res.status(200).json({
                message: "ProductionLine-Products relationship"
