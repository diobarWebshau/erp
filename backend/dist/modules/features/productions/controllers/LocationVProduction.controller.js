import LocationsControllers from "./../../../core/locations/controllers/index.js";
import { LocationModel, LocationTypeModel, LocationLocationTypeModel, LocationsProductionLinesModel, ProductionLineModel } from "../../../associations.js";
import { Op } from "sequelize";
class LocationVProductionController extends LocationsControllers.LocationController {
    ...
    static getProductionLines = async (req, res, next) => {
        const { id } = req.params;
        try {
            const validateLocation = await LocationModel.findOne({ where: { id: id } });
            if (!validateLocation) {
    ...
    static addProductionLine = async (req, res, next) => {
        ...
        try {
            ...
            const validationExistsProductionLineInSomeLocations = await LocationsProductionLinesModel.findOne({
                where: { production_line_id: production_line_id, }
            });
            if (validationExistsProductionLineInSomeLocations) {
                res.status(200).json({
                    validation: "The production line already has an assigned location"
                });
                return;
            }
            ...
            res.status(200).json({
                message: "The production line has been assigned to the location"
            });
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    static removeProductionLine = async (req, res, next) => {
        const { id, production_line_id } = req.params;
        try {
            const validateLocation = await LocationModel.findOne({ where: { id: id } });
            if (!validateLocation) {
