...
class InputController {
    ...
    static createComplete = async (req, res, next) => {
        ...
        try {
            ...
            if (validateName) {
                ...
                res.status(400).json({
                    validation: `The name is already`
    ...
    static updateComplete = async (req, res, next) => {
        ...
        try {
            ...
            const validateInput = await InputModel.findByPk(id);
            if (!validateInput) {
                await transaction.rollback();
            ...
            if (Object.keys(update_values).length > 0) {
                ...
                if (update_values?.input_types_id) {
                    const validateInputType = await InputTypeModel
                    ...
                    if (!validateInputType) {
                        ...
                        res.status(404).json({
                            validation: "Input type not found"
                        });
                        return;
                    }
                }
                ...
                if (!responseInput) {
                    ...
                    res.status(400).json({
                        validation: "The input could not be updated"
                    });
                    return;
                }
            }
    ...
    static delete = async (req, res, next) => {
        const { id } = req.params;
        try {
            const valiadDelete = await InputModel.findByPk(id);
            if (!valiadDelete) {
                res.status(200).json({
                    validation: "Input not found for deleted"
                });
                return;
            }
            const inputDelete = valiadDelete.toJSON();
            const validateDelete = await sequelize.query(`SELECT func_input_is_asigned_product(:id)` +
                ` AS is_asigned`, {
            ...
            if (responseValidation.is_asigned > 0) {
                res.status(400).json({
                    validation: `The input is currently assigned `
