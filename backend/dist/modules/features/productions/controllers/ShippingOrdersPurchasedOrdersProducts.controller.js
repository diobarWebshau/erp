...
import { ShippingOrderModel, PurchaseOrderProductModel, ShippingOrderPurchaseOrderProductModel, PurchasedOrderModel, PurchasedOrdersProductsLocationsProductionLinesModel, ProductionLineModel, LocationsProductionLinesModel, LocationModel, ProductionOrderModel } from "../../../associations.js";
import sequelize from "../../../../mysql/configSequelize.js";
class ShippingOrderPurchaseOrderProductController {
    static getAll = async (req, res, next) => {
        try {
            const response = await ShippingOrderPurchaseOrderProductModel.findAll();
            if (!(response.length > 0)) {
                res.status(200).json({
                    validation: "ShippingOrder-PurchaseOrderProduct "
    ...
    static getById = async (req, res, next) => {
        const { id } = req.params;
        try {
            const response = await ShippingOrderPurchaseOrderProductModel.findOne({
                where: { id: id }
            });
            if (!response) {
                res.status(200).json({
    ...
    static create = async (req, res, next) => {
        const { shipping_order_id, purchase_order_product_id, qty } = req.body;
        try {
            const validateShippingOrder = await ShippingOrderModel
            ...
            const validatePurchasedOrderProduct = await PurchaseOrderProductModel
                .findByPk(purchase_order_product_id);
            if (!validatePurchasedOrderProduct) {
                res.status(200).json({
                    validation: "The assigned purchase order "
            ...
            if (!(qty > 0)) {
                res.status(400).json({
                    validation: "The qty must be greater than zero"
                });
            }
            const isCompletedPop = validatePurchasedOrderProduct.toJSON();
            if (isCompletedPop.status === 'shipping') {
                res.status(200).json({
                    validation: `The purchased order product has already `
            ...
            //     return;
            // }
            const validationPurchasedOrderProductFullyShipping = await ShippingOrderPurchaseOrderProductModel
                .findAll({
                where: {
    ...
    static update = async (req, res, next) => {
        ...
        const body = req.body;
        try {
            const validateRelationship = await ShippingOrderPurchaseOrderProductModel.findByPk(id);
            if (!validateRelationship) {
                res.status(200).json({
                    validation: "ShippingOrder-PurchaseOrderProduct relationship no "
                        + "found for update"
                });
                return;
            }
            const relationship = validateRelationship.toJSON();
            const editableFields = ShippingOrderPurchaseOrderProductModel.getEditableFields();
            ...
            if (update_values?.purchase_order_product_id
                ...
                const [validateShippingOrder, validatePurchaseOrderProduct] = await Promise.all([
                    update_values?.shipping_order_id
                        ? ShippingOrderModel.findByPk(update_values.shipping_order_id)
                        : null,
                ...
                if (update_values?.shipping_order_id
                    ...
                    res.status(404).json({
                        validation: "The assigned shipping order does not exist"
                    });
                    return;
                }
                if (update_values?.purchase_order_product_id
                    ...
                    res.status(404).json({
                        validation: "The assigned purchase order product does not exist"
                    });
                    return;
                }
            }
            const validateShippingOrderClientOnShippingOrder = await ShippingOrderPurchaseOrderProductModel.findAll({
                where: {
                    shipping_order_id: update_values.shipping_order_id
            ...
            if (validateShippingOrderClientOnShippingOrder.length > 0) {
                const purchasedOrders = validateShippingOrderClientOnShippingOrder.map(sh => sh.toJSON());
                const purchased_order_details_aux = purchasedOrders.shift();
                const purchased_order_details = purchased_order_details_aux.purchase_order_products.purchase_order;
                const location_productionline_details = purchased_order_details_aux
                    .purchase_order_products
                    .purchase_order_product_location_production_line.production_line;
                const obtainClientPurchaseOrderProduct = await PurchaseOrderProductModel.findOne({
                    where: {
                ...
                const purchased_order_product_details = obtainClientPurchaseOrderProduct?.toJSON();
                if (purchased_order_product_details.status
                    != "completed") {
                    res.status(200).json({
                        validation: "The purchase order has not been completed yet."
                    });
                    return;
                }
                if (purchased_order_product_details.purchase_order.client_id
                    !== purchased_order_details.client_id) {
                    res.status(200).json({
                        validation: "The purchase order product does not belong to "
                            + "the same client as the shipping order"
                    });
                    return;
                }
            ...
            if (update_values?.purchase_order_product_id) {
                const validationPurchasedOrderProductDuplicate = await ShippingOrderPurchaseOrderProductModel.findOne({
                    where: {
                ...
                if (validationPurchasedOrderProductDuplicate) {
                    res.status(200).json({
                        validation: "The purchase order product has already been "
                            + "assigned to a shipping order"
                    });
                    return;
                }
            }
            const validateCreation = await ShippingOrderPurchaseOrderProductModel.findOne({
                where: {
                    [Op.and]: [
                        {
                            shipping_order_id: update_values.shipping_order_id
            ...
            if (validateCreation) {
                res.status(200).json({
                    validate: "ShippingOrder-PurchaseOrderProduct "
                        + "relationship already exists"
                });
                return;
            }
            const response = await ShippingOrderPurchaseOrderProductModel.update(update_values, { where: { id: id } });
            if (!(response[0] > 0)) {
                res.status(200).json({
    ...
    static deleteById = async (req, res, next) => {
        const { id } = req.params;
        try {
            const response = await ShippingOrderPurchaseOrderProductModel.destroy({
    ...
    static prueba = async (req, res, next) => {
        const { id } = req.params;
        try {
            ...
            const inventoryProductLocationResponse = await sequelize.query(`CALL getInventoryByLocation(); `, { type: QueryTypes.RAW });
            const inventoriesLocations = inventoryProductLocationResponse;
            ...
            const locationInventory = formattedLocations.find(inventory => inventory[0]?.location_id === 1);
            if (!locationInventory) {
                res.status(404).json({
            ...
            const inventories = locationInventory.filter(item => item.item_id === 1 &&
                item.item_type === 'product');
            const inventory = inventories.shift();
            const responseProduction = await ProductionOrderModel.findAll({
                where: {
            ...
            const data = responseProduction.map(K => K.toJSON());
            const qty_ordered = Number(data.reduce((acc, value) => acc + value.qty, 0));
