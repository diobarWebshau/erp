import { InternalProductProductionOrderModel, LocationModel, ProductModel } from "../../../associations.js";
...
class InternalProductProductionOrderController {
    ...
    static create = async (req, res, next) => {
        const { product_id, location_id, qty } = req.body;
        try {
            if (qty <= 0) {
                res.status(400).json({
            ...
            const [validateProduct, validateLocation] = await Promise.all([
            ...
            const asignedInternalOrderProductionLine = await sequelize.query("CALL asigned_internal_order_production_line"
            ...
            const isCapable = asignedInternalOrderProductionLine.shift();
            if (!isCapable || !isCapable.is_sufficient) {
                res.status(400).json({
                    validation: "The location does not have enough capacity"
                        + " to produce the requested quantity."
                });
                return;
            }
            const asignProductionLineResponse = await sequelize.query("SELECT asign_production_line"
            ...
            const production_line_id = asignProductionLineResponse.shift();
            if (!production_line_id || production_line_id.line <= 0) {
                res.status(400).json({
                    validation: "No production line at the specified" +
                        " location produces the indicated product."
                });
                return;
            }
            const response = await InternalProductProductionOrderModel.create({
                product_id,
                qty: qty,
                status: "pending",
            ...
            if (!response) {
                res.status(400).json({
                    message: "The internal product production order"
                        + " could not be created"
                });
                return;
            }
            const internalOrder = response.dataValues;
            await sequelize.query("CALL asing_internal_order" +
            ...
            res.status(200).json({
                message: "Internal product production order created successfully"
            });
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    static update = async (req, res, next) => {
        const { id } = req.params;
        const body = req.body;
        let production_line_id = 0;
        try {
            const validateInternalOrder = await InternalProductProductionOrderModel.findByPk(id);
            ...
            const editableFields = InternalProductProductionOrderModel.getEditableFields();
            ...
            const validateOrderHasProductionResponse = await sequelize.query("SELECT order_has_production"
            ...
            if (isContainBlockField && isOrderHasProduction === 1) {
                res.status(400).json({
                    validation: "You can't change the product or quantity "
            ...
            if (isContainBlockField && isOrderHasProduction === 0) {
                ...
                const productionLineObject = asignProductionLineResponse.shift();
            ...
            const response = await InternalProductProductionOrderModel.update(update_values, { where: { id: id }, individualHooks: true });
            if (response[0] === 0) {
                res.status(400).json({
                    validation: "No changes were made to the internal product production order."
                });
                return;
            }
            const validateInternalOrderReturn = await InternalProductProductionOrderModel.findByPk(id);
            if (isContainBlockField && isOrderHasProduction === 0) {
    ...
    static delete = async (req, res, next) => {
        ...
        try {
            ...
            if (!(response > 0)) {
                res.status(400).json({
                    validation: "The internal product production order could not be deleted."
                });
                return;
            }
            res.status(200).json({
    ...
    static revertProductionOfIppo = async (req, res, next) => {
        const { id } = req.params;
        try {
