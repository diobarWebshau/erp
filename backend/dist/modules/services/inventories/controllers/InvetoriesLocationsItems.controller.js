import { InventoryModel, InventoryLocationItemModel, LocationLocationTypeModel, LocationModel, LocationTypeModel, ProductModel, InputModel } from "../../../associations.js";
...
class InventoriesLocationsItemsController {
    static getAll = async (req, res, next) => {
        try {
            ...
            if (!(response.length > 0)) {
                res.status(404).json({
                    validation: "Item inventories not found at the specified location"
                });
                return;
            }
    ...
    static getById = async (req, res, next) => {
        ...
        try {
            ...
            if (!response) {
                res.status(404).json({
                    validation: "Item inventory not found at the specified location"
                });
                return;
            }
    ...
    */
    static create = async (req, res, next) => {
        ...
        try {
            ...
            if (!isStoreLocation) {
                res.status(400).json({
                    validation: "Inventory can only be assigned to locations of type 'Store'."
                });
                return;
            }
            ...
            if (!validItemTypes.includes(item_type)) {
                res.status(400).json({
                    validation: "Invalid item type. Only 'product' or 'input' are allowed."
                });
                return;
            }
            ...
            if (duplicateEntry) {
                res.status(409).json({
                    validation: "This item already has inventory at the specified location."
                });
                return;
            }
    ...
    */
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!existingRecord) {
                res.status(404).json({
                    validation: "The specified inventory record was not found."
                });
                return;
            }
            ...
            const [inventoryExists, inventoryAlreadyAssigned, location, locationWithType] = await Promise.all([
            ...
            if (update_values?.inventory_id && inventoryAlreadyAssigned) {
                res.status(409).json({
                    validation: "This inventory is already assigned to another location."
                });
                return;
            }
            ...
            if (update_values?.location_id && !locationWithType) {
                res.status(400).json({
                    validation: "Inventory can only be assigned to "
            ...
            if (!["product", "input"].includes(finalItemType)) {
                res.status(400).json({
                    validation: "The item type is invalid. Only 'product' "
            ...
            if (duplicateCombination) {
                res.status(400).json({
                    validation: "This combination of item_id, item_type and " +
                        "location_id is already in use."
                });
                return;
            }
            const result = await InventoryLocationItemModel.update(update_values, {
                where: { id },
    ...
    static delete = async (req, res, next) => {
        ...
        try {
            ...
            if (!response) {
                res.status(404).json({
                    validation: "No inventory record found for the "
                        + "specified item at the given location"
                });
                return;
            }
            res.status(200).json({
                message: "Inventory linkage between the specified item "
