...
import { InventoryTransferModel, ProductModel, LocationModel, LocationTypeModel, LocationLocationTypeModel, InventoryLocationItemModel, InventoryModel, InputModel } from "../../../associations.js";
...
class InventoryTransfersController {
    ...
    */
    static create = async (req, res, next) => {
        const { item_type, item_id, qty, reason, source_location_id, destination_location_id } = req.body;
        try {
            ...
            if (!(qty > 0)) {
                res.status(400).json({
                    validation: `The quantity must be greater `
            ...
            if (!sourceLocation) {
                res.status(404).json({
                    validation: `The source location does not exist`
                });
                return;
            }
            if (!destinationLocation) {
                res.status(404).json({
                    validation: "The destination location does not exist"
                });
                return;
            }
            const [sourceLocationType, destinationLocationType] = await Promise.all([
                LocationModel.findOne({
                    ...
                    include: {
                        ...
                        attributes: LocationLocationTypeModel
                            .getAllFields(),
                        include: [{
                                model: LocationTypeModel,
                                as: "location_type",
                                attributes: LocationTypeModel
                                    .getAllFields(),
                                where: { name: "Store" }
                            }]
                    }
            ...
            if (!sourceLocationType) {
                res.status(400).json({
                    validation: `The source location must`
                        + ` be of type store`
                });
                return;
            }
            if (!destinationLocationType) {
                res.status(400).json({
                    validation: `The destination location must`
                        + ` be of type store`
                });
                return;
            }
            if (item_type !== "product"
                && item_type !== "input") {
                res.status(400).json({
                    validation: `The item type is not valid`
                });
                return;
            }
            let itemModel;
            ...
            const [sourceLocationResponse, destinationLocationResponse] = await Promise.all([
                LocationModel.findOne({
                    where: { id: source_location_id },
                    attributes: LocationModel
                        .getAllFields(),
                    include: [{
                            ...
                            attributes: InventoryLocationItemModel
                                .getAllFields(),
                            include: [{
            ...
            const sourceLocationInventoryItem = sourceLocationResponse?.toJSON() || null;
            const destinationLocationInventoryItem = destinationLocationResponse?.toJSON() || null;
            if (!sourceLocationInventoryItem) {
                res.status(404).json({
                    validation: `Item not found in source` +
                        ` location inventory`
                });
                return;
            }
            if (!destinationLocationInventoryItem) {
                res.status(404).json({
                    validation: `Item not found in destination 
                        ` + `location inventory`
                });
                return;
            }
            ...
            if (!sourceInventory || sourceInventory.stock < qty) {
                res.status(400).json({
                    validation: `Not enough quantity available in source location. `
            ...
            //     return;
            // }
            const newTransfer = await InventoryTransferModel.create({
                item_type,
                item_id,
            ...
            if (!newTransfer) {
                res.status(400).json({
                    validation: "Inventory transfers could not be created"
                });
                return;
            }
            res.status(201).json({
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(400).json({
                    validation: "There are no validated inventory transfer properties for the update"
                });
                return;
            }
            if (update_values?.status) {
                if (relationships.status === "completed" &&
                    ...
                    res.status(200).json({
                        validation: "The inventory transfer is already marked as completed"
                    });
                    return;
                }
                    ...
                    res.status(200).json({
                        validation: "The inventory transfer is already canceled"
                    });
                    return;
                }
                else if (relationships.status === "canceled" &&
                    update_values.status === "completed") {
                    res.status(400).json({
                        validation: "A canceled inventory transfer cannot be marked as completed"
                    });
                    return;
                }
            }
            const { source_location_id, destination_location_id, qty, item_type, item_id } = relationships;
            ...
            const sourceLocationInventoryItem = results[0];
            const destinationLocationInventoryItem = results[1];
            if (!sourceLocationInventoryItem) {
                res.status(200).json({
            ...
            const sourceInventoryItem = sourceLocationInventoryItem.toJSON().inventory_location_item[0];
            const sourceInventory = sourceInventoryItem.inventory;
            const destinationInventoryItem = destinationLocationInventoryItem.toJSON().inventory_location_item[0];
            const destinationInventory = destinationInventoryItem.inventory;
            if (!destinationInventory || destinationInventory.stock < qty) {
                res.status(400).json({
                    validation: `Not enough quantity available in the destination location to revert the transfer. `
            ...
            if (Number(sourceInventory.maximum_stock) < Number(sourceInventory.stock) + Number(qty)) {
                res.status(400).json({
                    validation: `Cannot revert the transfer because it would exceed the maximum stock allowed in the source location. `
                        + `Maximum allowed: ${sourceInventory.maximum_stock}, Resulting stock: ${sourceInventory.stock + qty}`
                });
                return;
            }
            const response = await InventoryTransferModel.update(update_values, {
                where: { id: id },
                individualHooks: true
            });
            if (!(response[0] > 0)) {
                res.status(200).json({
                    message: "No changes were made to the inventory transfers"
                });
                return;
            }
    ...
    static delete = async (req, res, next) => {
        ...
        try {
            ...
            if (relationships.status !== "canceled") {
                ...
                if (!destinationInventory || destinationInventory.stock < qty) {
                    res.status(400).json({
                        validation: `Deletion blocked: Not enough quantity available in the `
                ...
                if (Number(sourceInventory.maximum_stock) < Number(sourceInventory.stock) + Number(qty)) {
                    res.status(400).json({
                        validation: `Deletion blocked: Cannot revert the transfer because it `
                ...
                await sequelize.query(`CALL revert_inventory_transfer(:transferId)`, {
                    replacements: {
                        transferId: id
                    }
                });
            ...
            if (!(response > 0)) {
                res.status(400).json({
                    validation: "Inventory transfers record could not be deleted"
                });
                return;
            }
            res.status(200).json({
