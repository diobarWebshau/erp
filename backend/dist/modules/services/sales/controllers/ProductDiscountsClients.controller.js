...
import { ClientModel, ProductDiscountClientModel, ProductModel } from "../../../associations.js";
    ...
    static create = async (req, res, next) => {
        ...
        try {
            ...
            if (validateDuplicateDiscount) {
                res.status(409).json({
                    validation: "The discount percentage is already defined for "
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!productDiscountClient) {
                res.status(404).json({
                    validation: "Product discount client not found for update"
                });
                return;
            }
            ...
            if (Object.keys(update_values).length === 0) {
                res.status(404).json({
                    validation: "There are no validated client discount properties for the update"
                });
                return;
            }
            ...
            if (discountToCheck) {
                ...
                const existsSameDiscount = discounts.some(dp => Number(dp.discount_percentage) === Number(discountToCheck));
                if (existsSameDiscount) {
                    res.status(400).json({
                        validation: "The discount percentages are already defined in another "
                            + "client discount for the same client"
                    });
                    return;
                }
            }
            const [affectedRows] = await ProductDiscountClientModel.update(update_values, { where: { id }, individualHooks: true });
            if (affectedRows === 0) {
                res.status(200).json({
                    validation: "No changes were made to the product discount client"
                });
                return;
            }
