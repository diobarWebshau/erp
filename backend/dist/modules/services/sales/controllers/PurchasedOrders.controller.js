...
class PurchasedOrderController {
    ...
    static getByIds = async (req, res, next) => {
    ...
    static getByClientId = async (req, res, next) => {
    ...
    static create = async (req, res, next) => {
        const { order_code, delivery_date, status, client_id, client_address_id, total_price, created_at } = req.body;
        try {
            const valiadateOrderCode = await PurchasedOrderModel.findOne({ where: { order_code: order_code } });
            ...
            const validateClientId = await ClientModel.findByPk(client_id);
            if (!validateClientId) {
                res.status(404).json({
            ...
            const validateClientsAddress = await ClientAddressesModel.findByPk(client_address_id);
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            const response = await PurchasedOrderModel.update(update_values, { where: { id }, individualHooks: true });
            if (response[0] === 0) {
