...
import { LogModel, OperationModel, TableModel } from "../associations.js";
...
class LogsController {
    ...
    static getByOperationName = async (req, res, next) => {
    ...
    static getByTableName = async (req, res, next) => {
    ...
    static getByUserName = async (req, res, next) => {
    ...
    static create = async (req, res, next) => {
        const { table_id, user_id, operation_id, old_data, new_data, message } = req.body;
        try {
            const [validatedTable, validatedOperation, validateUser] = await Promise.all([
            ...
            if (!validatedTable) {
                res.status(200).json({
                    validation: "The table assigned does not exist"
                });
                return;
            }
            if (!validatedOperation) {
                res.status(200).json({
                    validation: "The operation assigned does not exist"
                });
                return;
            }
            if (!validateUser) {
                res.status(200).json({
                    validation: "The user assigned does not exist"
                });
                return;
            }
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation: "There are no validated log properties for the update"
                });
                return;
            }
            const response = await LogModel.update(update_values, { where: { id: id } });
            if (!(response[0] > 0)) {
