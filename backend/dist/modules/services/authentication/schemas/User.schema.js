...
const userSchema = zod.object({
    ...
    password: zod.string().regex(passwordRegEx, "Password must contain at least one uppercase "
...
const validatePartialSafeParseAsync = async (input) => {
    const result = await userSchema.partial().safeParseAsync(input);
    return result;
};
export { validateSafeParse, validateSafeParseAsync, validatePartialSafeParse, validatePartialSafeParseAsync, userSchema };
