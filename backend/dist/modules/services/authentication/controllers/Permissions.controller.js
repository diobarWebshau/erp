...
import { PermissionModel, RoleModel } from "../associations.js";
...
class PermissionController {
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation: "There are no validated permission properties for the update"
                });
                return;
            }
            ...
            const response = await PermissionModel.update(update_values, {
                where: { id: id },
    ...
    static getRolesWithPermission = async (req, res, next) => {
        const { id } = req.params;
        try {
            const validatedPermission = await PermissionModel.findByPk(id);
            if (!validatedPermission) {
