...
import { UserModel, RoleModel } from "../associations.js";
import { generateHash } from "../../../../utils/argon/argon.js";
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation: "There are no validated user properties for the update"
                });
                return;
            }
            ...
            const response = await UserModel.update(update_values, { where: { id: id }, individualHooks: true });
            if (!(response[0] > 0)) {
                res.status(200).json({
