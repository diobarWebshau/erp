...
import SalesControllers
    from "../../../services/sales/controllers/index.js";
import sequelize
...
import {
    ...
    ClientModel, ProductDiscountClientModel,
...
} from "./../../../services/sales/models/junctions/purchased-orders-products.model.js";
import { ProductAttributes, PurchasedOrderCreateAttributes } from "../../../types.js";
...
class PurchasedOrdersVProduction extends SalesControllers.PurchasedOrderController {
    ...
    static getProductsOrderProductByClientAddress = async (req: Request, res: Response, next: NextFunction) => {
    ...
    static getProductsInOrder = async (req: Request, res: Response, next: NextFunction) => {
        const { id } = req.params;
        try {
            const validatePurchaseOrder =
                await PurchasedOrderModel.findByPk(id);
            ...
            if (!response) {
                res.status(200).json({
                    validation:
                        "Products not found for this purchased order"
                });
                return;
            }
    ...
    static addProductOnOrder = async (req: Request, res: Response, next: NextFunction) => {
        const { id, product_id, } = req.params;
        const { qty, status } = req.body;
        try {
            const validate_purchase_order_id =
                await PurchasedOrderModel.findOne({
                    where: { id: id }
                });
            ...
            const validateproduct =
                await ProductModel.findOne({ where: { id: product_id } });
            if (!validateproduct) {
    ...
    static addProductsOnOrder = async (req: Request, res: Response, next: NextFunction) => {
        const { id } = req.params;
    ...
    static removeProductsOnOrder = async (req: Request, res: Response, next: NextFunction) => {
        const { id } = req.params;
    ...
    static removeProductOfOrder = async (req: Request, res: Response, next: NextFunction) => {
        const { id, product_id } = req.params;
        try {
    ...
    static getAllDetailsOfOrder = async (req: Request, res: Response, next: NextFunction) => {
        const { id } = req.params;
        try {
            const validatePurchaseOrder = await PurchasedOrderModel.findOne({ where: { id: id } });
            if (!validatePurchaseOrder) {
    ...
    static createComplete2 = async (
        ...
        try {
            ...
            const [validateClientId, validateClientsAddress] =
                await Promise.all([
            ...
            const client =
                validateClientId.toJSON();
            const clientAddress =
                validateClientsAddress.toJSON();
            ...
            const purchased_order_id: number =
                response.toJSON().id;
            ...
            if (!Array.isArray(purchase_order_products)
                ...
                res.status(200).json({
                    validation:
                        "Invalid or empty product list."
                });
                return;
            }

            const productIds =
                purchase_order_products.map(p => Number(p.product_id));
            ...
            if (validateExistingProductsOnOrders.length > 0) {
                ...
                res.status(200).json({
                    validation:
                        "One or more products already exists "
                        + "on Purcharsed order"
                });
                return;
            }
            ...
            const wasEditedMap = new Map<number, {
            ...
            const productToAdd: ProductAttributes[] =
                existingProducts.map(p => p.toJSON());

            const newProductsToOrders =
                purchase_order_products.map((value: PurchaseOrderProductCreateAttributes) => {
                    const product = productToAdd.find((product) => {
                ...
                }) as PurchaseOrderProductCreateAttributes[];

            const responseCreatePops:
                PurchaseOrderProductModel[] =
            ...
            if (!responseCreatePops) {
                ...
                res.status(200).json({
                    validation:
                        "Some Products could not be added to "
                        + "the Purchased Order"
                });
                return;
            }

            const responseCreatePopsJSON:
                PurchaseOrderProductAttributes[] =
                responseCreatePops.map(p => p.toJSON());
            ...
            for (const p of responseCreatePopsJSON) {
                ...
                if (
                    ...
                    const discountRange =
                        responseDiscountRange?.toJSON() ?? null;
                    const discountClient =
                        responseDiscountClient?.toJSON() ?? null;
            ...
            const newPurcahsedOrderProduct: number[] =
                responseCreatePopsJSON.map(p => p.id);


            // La respuesta es [ [ { conn_id: number } ], ... ]
            const responsesdadas = await sequelize.query(
            ...
            if (true) {
                await sequelize.query(
                    "CALL process_purchased_order_product_multiple(:pop_ids_json)",  // usa el nombre real del parámetro
                    {
                        replacements: {
            ...
            const purchasedOrderFeedBackJSON =
                await responsePurchasedOrderFeedBack?.toJSON();
    ...
    static updateComplete = async (
        ...
        const { id } = req.params;
        const completeBody: PurchasedOrderCreateAttributes =
            req.body as PurchasedOrderCreateAttributes;

        try {
            // * obtener los campos editables
            const editableFields =
                PurchasedOrderModel.getEditableFields();

            // * filtrar los campos editables del body
            const update_values: PurchasedOrderCreateAttributes =
                collectorUpdateFields(
                    editableFields,
                    req.body
                ) as PurchasedOrderCreateAttributes;

            // * validar que la orden de compra a actualizar exista
            const responseValidatePurchaseOrder =
                await PurchasedOrderModel.findByPk(
                    id,
                    { transaction }
                );
            if (!responseValidatePurchaseOrder) {
                ...
                res.status(404).json({
                    validation:
                        "Purchased order does not exist"
                });
                return;
            }
            const purchaseOrder =
                responseValidatePurchaseOrder.toJSON();
            ...
            if (Object.keys(update_values)?.length > 0) {
                ...
                if (update_values?.client_id) {
                    const responseValidateClient =
                        await ClientModel.findByPk(
                ...
                if (update_values?.client_address_id) {
                    const responseValidateClientAddress =
                        await ClientAddressesModel.findByPk(
                            update_values.client_address_id,
                            { transaction }
                        );
            ...
            const responseUpdatePurchasedOrder =
                await PurchasedOrderModel.update({
                    ...update_values,
                }, {
                    transaction,
            ...
            if (completeBody?.purchase_order_product_manager) {
                ...
                const popManager =
                    completeBody?.purchase_order_product_manager;

                // * validar si hay productos para actualizar
                const flagPopUpdate: boolean = [
                    popManager.added,
                    popManager.deleted,
                    popManager.modified
                ].some((p) => p?.length > 0);
                ...
                if (flagPopUpdate) {
                    ...
                    const added: PurchaseOrderProductCreateAttributes[] =
                        popManager.added;
                    const deleted: PurchaseOrderProductAttributes[] =
                        popManager.deleted;
                    const modified: PurchaseOrderProductCreateAttributes[] =
                        popManager.modified;

                    // * validar que existan productos a eliminar
                    if (deleted?.length > 0) {
                        // * obtener los productos validos para eliminar
                        const deleteFiltered: PurchaseOrderProductAttributes[] =
                            deleted.filter(d => d.id !== undefined);
                        ...
                        if (responseExistingPops.length !== deleteFiltered.length) {
                            ...
                            res.status(404).json({
                                validation:
                                    `Some products of the purchase order you`
                    ...
                    if (modified.length > 0) {
                        ...
                        const modifiedFiltered: PurchaseOrderProductCreateAttributes[] =
                        ...
                        const responseExistingPops =
                            await PurchaseOrderProductModel.findAll({
                                where: {
                                    id: { [Op.in]: modifiedFiltered.map(d => d.id as number) }
                                }, transaction,
                                lock: transaction.LOCK.SHARE,
                            });
                        console.log(responseExistingPops);
                        ...
                        for (const pop of modifiedFiltered) {
                            ...
                            const responsePurchasedOrderProduct =
                            ...
                            const purchasedOrderProduct =
                                responsePurchasedOrderProduct.toJSON();
                    ...
                    if (added.length > 0) {
                        ...
                        const popProductId: number[] =
                            addsFiltered.map(d => d.product_id);

                        // * validamos que los productos existan
                        const responseExistingProducts =
                            await ProductModel.findAll({
                                where: {
                        ...
                        if (responseExistingProducts.length !== popProductId.length) {
                            ...
                            res.status(200).json({
                                validation:
                                    `One or more products tried`
                                    + ` to add do not exist`
                            });
                            return;
                        }
                        const productToAdd: ProductAttributes[] =
                            responseExistingProducts.map(p => p.toJSON());

                        // * Validamos que los productos no esten repetidos
                        const responseValidateExistingProductsOnOrders =
                            await PurchaseOrderProductModel.findAll({
                                where: {
                        ...
                        const newProductsToOrders =
                            addsFiltered.map((value: PurchaseOrderProductCreateAttributes) => {
                                const product = productToAdd.find((product) => {
                                    if (Number(product.id) ===
                                        Number(value.product_id))
                                        return product;
                        ...
                        if (true) {
                            await sequelize.query(
                                `CALL process_purchased_order_product_multiple(:ids)`,
                                {
                                    replacements: {
                                        ids:
                                            JSON.stringify(
                                                newPurcahsedOrderProduct
                                            )  // convertimos el array a una instancia de un objeto JSON
                                    },
                                    transaction
                                }
                            );
    ...
    static deleteWhenNoProduction = async (req: Request, res: Response, next: NextFunction) => {
        const { id } = req.params;
        try {
            ...
            const result = validationDelete.shift() as { is_produced: number };
