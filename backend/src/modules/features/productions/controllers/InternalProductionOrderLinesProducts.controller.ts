...
class InternalProductionOrderLineProductController {
    static getAll = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const response =
                await InternalProductionOrderLineProductModel.findAll();
            if (!(response.length > 0)) {
                res.status(200).json({
                    validaton:
                        "No internal production orders assigned" +
    ...
    static getById = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const response =
                await InternalProductionOrderLineProductModel.findOne({
                    where: { id: id }
                });
            if (!response) {
                res.status(200).json({
                    validation:
                        "No internal production order " +
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const [
                validateInternalProductProductionOrder,
                validateLocationProductionLine
            ] = await Promise.all([
                InternalProductProductionOrderModel.
            ...
            if (!validateInternalProductProductionOrder) {
                res.status(200).json({
                    validation:
                        "No internal product production order was found"
                });
                return;
            }
            if (!validateLocationProductionLine) {
                res.status(200).json({
                    validation:
                        "The specified production line could not be found."
                });
                return;
            }
            const internal_product_production_order =
                validateInternalProductProductionOrder.toJSON();
            const validateProductInProductionLine =
            ...
            if (!validateProductInProductionLine) {
                res.status(200).json({
                    validation:
                        "The product is not associated with the specified"
                        + " production line."
                });
                return;
            }

            const validateInternalProductProductionOrderDuplicate =
                await InternalProductionOrderLineProductModel.findOne({
                    where: {
                        internal_product_production_order_id
                    }
            ...
            if (validateInternalProductProductionOrderDuplicate) {
                res.status(409).json({
                    validation:
                        "The internal production order is already assigned to "
            ...
            const response = await
                InternalProductionOrderLineProductModel.create({
                    internal_product_production_order_id,
                    production_line_id
                });
            ...
            if (!response) {
                res.status(500).json({
                    validation:
                        "The internal production order could not be " +
            ...
            res.status(200).json({
                message:
                    "The internal production order has been successfully " +
                    "assigned to the production line."
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            const validateInternalProductProductionOrder =
                await InternalProductionOrderLineProductModel.findOne({
                    where: { id: id }
                });
            ...
            if (!validateInternalProductProductionOrder) {
                res.status(404).json({
                    validation:
                        "No internal product production order assigned to "
            ...
            if (update_values?.production_line_id &&
                ...
                const duplicate = await
                    InternalProductionOrderLineProductModel.findOne({
                        where: {
                            production_line_id:
                                update_values
                                    .production_line_id,
                ...
                if (duplicate) {
                    res.status(409).json({
                        validation:
                            "This combination of internal production order and "
                            + "production line is already assigned."
                    });
                    return;
                }
            } else if (update_values?.internal_product_production_order_id) {
                const internalOrder = await
                    InternalProductProductionOrderModel.findByPk(
                        update_values.internal_product_production_order_id
                    );
                if (!internalOrder) {
                    res.status(404).json({
                ...
                const duplicate = await InternalProductionOrderLineProductModel.findOne({
                    where: {
                        [Op.and]: [
                            ...
                            {
                                production_line_id:
                                    relationships.production_line_id
                            },
                            {
                                id: {
                                    [Op.ne]: id
                ...
                if (duplicate) {
                    res.status(409).json({
                        validation:
                            "This internal production order is already assigned "
                ...
                const location = await ProductionLineModel.findByPk(
                    update_values.production_line_id
                );
                if (!location) {
                    res.status(404).json({
                        validation:
                            "The specified production line location does not exist."
                    });
                    return;
                }
                const duplicate = await InternalProductionOrderLineProductModel.findOne({
                    where: {
                        [Op.and]: [
                            {
                                internal_product_production_order_id:
                                    relationships.internal_product_production_order_id
                            },
                            {
                                production_line_id:
                                    update_values.production_line_id
                ...
                if (duplicate) {
                    res.status(409).json({
                        validation:
                            "The specified production line is already assigned to this "
                            + "internal production order."
                    });
                    return;
                }
            }
            const response = await InternalProductionOrderLineProductModel.update(
                update_values,
            ...
            if (response[0] === 0) {
                res.status(400).json({
                    validation:
                        "No changes were made. The data may already "
                        + "be up to date."
                });
                return;
            }
            ...
            res.status(200).json({
                message:
                    "The internal production order assignment "
    ...
    static delete = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const response = await
                InternalProductionOrderLineProductModel.destroy({
                    where: { id: id }, individualHooks: true
                });
            ...
            if (response === 0) {
                res.status(404).json({
                    validation:
                        "No internal production order assignment "
            ...
            res.status(200).json({
                message:
                    "The internal production order assignment has "
