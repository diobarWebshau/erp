...
import { ProductModel, ProcessModel, ProductProcessModel }
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (validation) {
                res.status(200).json({
                    validation:
                        "Product-process relationship already exists"
                });
                return;
            }
            console.log("entro")
            ...
            if (!response) {
                res.status(200).json({
                    validation:
                        "The product-process relationship could not be created"
                });
                return;
            }
            res.status(200).json({
                message:
                    "Product-process relationship created successfully"
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
                next(error);
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!validateRelationship) {
                res.status(200).json({
                    validation:
                        "Product-process relationship no found for update"
                });
                return;
            }
            ...
            if (validateDuplicate) {
                res.status(200).json({
                    validation:
                        "The product-process relationship already exists"
                })
                return;
            }
            const response = await ProductProcessModel.update(
                update_values,
            ...
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation:
                        "No changes were made to the product-process relationship"
                });
                return;
            }
            res.status(200).json({
    ...
    static deleteById = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!(response > 0)) {
                res.status(200).json({
                    validation:
                        "Products-process relationship no found for delete"
                });
                return;
            }
            res.status(200).json({
