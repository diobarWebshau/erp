...
import {
    ...
    LocationsProductionLinesModel, PurchaseOrderProductModel,
    ProductionLineModel
...
class PurchasedOrdersProductsLocationsProductionLinesController {
    static getAll = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const response = await
                PurchasedOrdersProductsLocationsProductionLinesModel.findAll();
            if (!(response.length > 0)) {
                res.status(404).json({
                    validation:
                        "PurchasedOrdersProducts-LocationsProductionLines" +
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            const [validateLocationProductionLine, validatePurchasedOrderProduct] =
                await Promise.all([
            ...
            if (!validateLocationProductionLine) {
                res.status(404).json({
                    validation:
                        "The assigned locations-production-lines does not exist"
                });
                return;
            }
            if (!validatePurchasedOrderProduct) {
                res.status(404).json({
                    validation:
                        "The assigned purchased order product does not exist"
                });
                return;
            }
            const validateDuplicate = await
                PurchasedOrdersProductsLocationsProductionLinesModel.findOne({
                    where: {
                        [Op.and]: [
                            {
                                production_line_id:
            ...
            if (validateDuplicate) {
                res.status(409).json({
                    validation:
                        "PurchasedOrderProducts-LocationsProductionLines"
                        + " relationship already exists"
                });
                return;
            }
            const validatePurchasedOrderProductInSomeLocationProductionLine =
                await PurchasedOrdersProductsLocationsProductionLinesModel
                    .findOne({
                        where: {
                            purchase_order_product_id:
                                purchase_order_product_id
            ...
            if (validatePurchasedOrderProductInSomeLocationProductionLine) {
                res.status(409).json({
                    validaiton:
                        "The purchased order product has already been"
            ...
            if (!response) {
                res.status(400).json({
                    validation:
                        "The Purchased-orders-products-Location"
            ...
            res.status(201).json({
                message:
                    "The purchased order product has been "
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const body = req.body;
        try {
            const validateRelationship = await
                PurchasedOrdersProductsLocationsProductionLinesModel
                    .findByPk(id);
            if (!validateRelationship) {
                res.status(404).json({
                    validation:
                        "PurchasedOrdersProducts-Locations"
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(400).json({
                    validation:
                        "There are no validated purchased order"
            ...
            const validatePurchasedOrderProductInSomeProductionLine =
                await PurchasedOrdersProductsLocationsProductionLinesModel
                    .findOne({
                        where: {
                            [Op.and]: [
            ...
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation:
                        "No changes were made to the PurchasedOrdersProducts"
    ...
    static deleteById = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const response = await
                PurchasedOrdersProductsLocationsProductionLinesModel.destroy({
                    where: { id: id }, individualHooks: true
                });
            if (!(response > 0)) {
                res.status(200).json({
