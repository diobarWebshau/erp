import collectorUpdateFields from
    "./../../../../scripts/collectorUpdateField.js";
import {
...
class InvetoryMovementsController {
        ...
        async (req: Request, res: Response, next: NextFunction) => {
            try {
                const response = await
                    InventoryMovementModel.findAll();
                if (!(response.length > 0)) {
                    res.status(404).json({
                        validation:
                            'Inventory movements no found'
                    });
                    return;
                }
                const inventoryMovements =
                    response.map(c => c.toJSON());
    ...
    static getById =
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            try {
                const response =
                    await InventoryMovementModel.findByPk(id);
                if (!response) {
                    res.status(404).json({
                        validation:
                            'Inventory movement no found'
                    });
                    return;
                }
    ...
    static create =
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            try {
                ...
                let location_name, item_name: string;
                let itemModel: ModelStatic<Model>;
                ...
                if (!response) {
                    res.status(400).json({
                        validation:
                            'Inventory movement could not be created'
                    });
                    return;
                }
                ...
                if (true) {
                    await sequelize.query(
                        `CALL process_waiting_purchased_orders_products();`
                    );
                }
    ...
    static update =
        async (req: Request, res: Response, next: NextFunction) => {
            ...
            try {
                ...
                const relationships =
                    validateInventoryMovement.toJSON();
                const editableFields =
                    InventoryMovementModel.getEditableFields();
                const update_values =
                    collectorUpdateFields(editableFields, body);
                if (Object.keys(update_values).length === 0) {
                    res.status(400).json({
                        validation:
                            "No validated inventory movements "
                ...
                if (update_values?.is_locked) {
                    if (relationships.is_locked === 0) {
                        res.status(400).json({
                            validation:
                                "Inventory movement was already completed"
                        });
                        return;
                    }
                }
                ...
                if (!response) {
                    res.status(400).json({
                        validation:
                            'Inventory movement could not be updated'
                    });
                    return;
                }
