import { ProductDiscountRangeModel, ProductModel }
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!validatedproduct) {
                res.status(200).json({
                    validation:
                        "The product assiged to the discount range does not exist"
                });
                return;
            }
            ...
            if (validateRangeDiscount.length > 0) {
                ...
                if (!NoduplicatedRange) {
                    res.status(200).json({
                        validation:
                            "The discount range already exists or overlaps with another "
            ...
            if (!response) {
                res.status(200).json({
                    validation:
                        "Could not create a discount range for the product"
                });
            }
            res.status(200).json({
                message:
                    "Discount range created successfully for the product"
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
                next(error);
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (Object.keys(update_values).length === 0) {
                res.status(400).json({
                    validation:
                        "There are no validated product discount " +
                        "ranges properties for the update."
                });
                return;
            }
            const validateProductDiscount =
                await ProductDiscountRangeModel.findByPk(id);
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation:
                        "There are no validated product discount ranges properties "
                        + "for the update"
                });
                return;
            }
            ...
            if (validateRangeDiscount.length > 0) {
                ...
                const NoduplicatedRange = rangeDiscount.every((element, index) => {
                    ...
                    const max_qty_aux = element.max_qty;
                    return (
                        (update_values.min_qty >= max_qty_aux ||
                ...
                if (!NoduplicatedRange) {
                    res.status(200).json({
                        validation:
                            "The discount range already exists or overlaps with "
            ...
            const response = await ProductDiscountRangeModel.update(
                update_values,
                {
                    where: { id: id },
            ...
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation:
                        "No product discount ranges found to update or not "
                        + "changes were made"
                });
                return;
            }
            res.status(200).json({
                message:
                    "Product discount ranges updated successfully"
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
                next(error);
