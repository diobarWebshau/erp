...
import { LocationTypeModel, LocationLocationTypeModel, LocationModel }
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (validateName) {
                res.status(409).json({
                    validation:
                        "The name is already currently in use by a locationType"
                });
                return;
            }
            ...
            if (!response) {
                res.status(400).json({
                    validation:
                        "The locationType could not be created"
                });
                return;
            }
            res.status(201).json({
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const editableFields =
                LocationTypeModel.getEditableFields();
            const update_values =
                collectorUpdateFields(editableFields, body);
            if (Object.keys(update_values).length === 0) {
                res.status(400).json({
                    validation:
                        "There are no validated location "
                        + "type properties for the update."
                });
                return;
            }
            const validatedLocationType =
                await LocationTypeModel.findByPk(id);
            ...
            const response = await LocationTypeModel.update(
                update_values,
                {
                    where: { id: id },
    ...
    static delete = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (validationLocationTypes.length > 0) {
                res.status(404).json({
                    validation:
                        "This location type cannot be deleted because"
                        + " it is assigned to one or more locations."
                });
                return;
            }
            const response = await
                LocationTypeModel.destroy({
                    where: { id: id },
                    individualHooks: true
                });
            if (!(response > 0)) {
