...
const validateLocationsMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    const body = req.body;
    const method = req.method;
    try {
        console.log(`req.body middle - `, req.body);
        let result;
        if (method === "POST") result = await validateSafeParseAsync(body);
        else result = await validatePartialSafeParseAsync(body);
