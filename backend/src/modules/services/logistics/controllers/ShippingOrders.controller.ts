...
class ShippingOrderController {
    ...
    static createComplete = async (
        ...
        try {
            ...
            const pop: ShippingOrderPurchaseOrderProductCreateAttributes[] =
                purchase_order_products.map(({ id, ...rest }) => ({
                    ...rest,
    ...
    static updateComplete = async (
        ...
        try {
            ...
            let load_evidence_deleted: LoadEvidenceItem[] = (() => {
                try {
            ...
            const update_values_process: ShippingOrderCreationAttributes = {
                ...update_values,
                shipping_date: coerceValue(update_values.shipping_date),
            }

            const isEvidenceUpdate =
                completeBody.load_evidence_deleted && JSON.parse(completeBody.load_evidence_deleted).length > 0 ||
            ...
            if (completeBody?.shipping_order_purchase_order_product_manager) {
                ...
                if (flagProductsInputsUpdate) {
                    ...
                    if (modified.length > 0) {
                        ...
                        const popsiD: number[] = sopopsJson.map(sopop => sopop.purchase_order_product_id);
                    ...
                    if (adds.length > 0) {
                        ...
                        for (const p of new_pops) {
                            ...
                            const new_sopop: ShippingOrderPurchaseOrderProductCreateAttributes = {
                                ...found,
