    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation:
                        "There are no validated table properties for the update"
                });
                return;
            }
            if (update_values?.name) {
                ...
                if (validateName) {
                    res.status(200).json({
                        validation:
                            "The name is already used in a table"
                    });
                    return;
                }
            }
            if (update_values?.table_name) {
                ...
                if (validateTableName) {
                    res.status(200).json({
                        validation:
                            "The table name is already used in a table"
                    });
                    return;
                }
            }
            const response = await TableModel.update(
                update_values,
                { where: { id: id } }
            );
            if (!(response[0] > 0)) {
