...
class AppliedProductDiscountsRangesController {
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            const validateExistsPurchaseOrderProductWithDiscount =
                await AppliedProductDiscountRangeModel.findOne({
            ...
            const validateAppliedProductDiscount =
                await AppliedProductDiscountRangeModel.findOne({
                    where: {
                        [Op.and]: [
            ...
            if (validateAppliedProductDiscount) {
                res.status(200).json({
                    validation:
                        "The purchase order alreay has an applied client discount"
                });
                return;
            }
            ...
            if (!response) {
                res.status(200).json({
                    validation:
                        "The applied product discount could not be craeted"
                });
                return;
            }
            res.status(200).json({
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const body = req.body;
        try {
            const validateExistingPOAppliedProductDiscount =
                await AppliedProductDiscountRangeModel.findByPk(id);
            if (!validateExistingPOAppliedProductDiscount) {
                res.status(200).json({
                    validation:
                        "The applied product discount does not exist"
                });
                return;
            }
            ...
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation:
                        "There are no validated applied product "
                        + "discount properties for the update"
                });
                return;
            }
            const relationship =
                validateExistingPOAppliedProductDiscount.toJSON();

            if (update_values?.product_discount_range_id
                || update_values?.purchase_order_product_id) {
                const [
                    validateProductDiscount, validatePurchaseOrderProduct
                ] = await Promise.all([
            ...
            const response = await AppliedProductDiscountRangeModel.update(
                update_values,
                {
                    where: { id: id },
            ...
            res.status(200).json({
                message:
                    "The applied product discount was updated successfully"
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
                next(error);
