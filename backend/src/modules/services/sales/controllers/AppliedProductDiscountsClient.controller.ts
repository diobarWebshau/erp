...
import {
    AppliedProductDiscountClientModel, ProductDiscountClientModel,
    PurchasedOrderModel, PurchaseOrderProductModel
...
class AppliedProductDiscountClientController {
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            const validateExistingPOAppliedClientDiscount =
                await AppliedProductDiscountClientModel.findOne({
                    where: { purchase_order_product_id: purchase_order_product_id }
                });
            if (validateExistingPOAppliedClientDiscount) {
                res.status(200).json({
                    validation:
                        "The purchase order product already has an applied product discount client"
                });
                return;
            }
    ...
    static update = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const validateExistingPOAppliedClientDiscount =
                await AppliedProductDiscountClientModel.findByPk(id);
            if (!validateExistingPOAppliedClientDiscount) {
                res.status(200).json({
                    validation:
                        "The applied product discount client does not exist"
                });
                return;
            }
            const relationship =
                validateExistingPOAppliedClientDiscount.toJSON();
            const editableFields =
                AppliedProductDiscountClientModel.getEditableFields();
            const update_values = collectorUpdateFields(editableFields, body);
            if (!(Object.keys(update_values).length > 0)) {
                res.status(200).json({
                    validation:
                        "There are no validated applied product discount "
            ...
            if (update_values?.product_discount_client_id
                ...
                const [
                    validateProductDiscountClient, validatePurchaseOrderProduct
                ] = await Promise.all([
            ...
            if (validateDuplicate) {
                res.status(200).json({
                    validation:
                        "The applied product discount client already exists"
                });
                return;
            }
            const response = await AppliedProductDiscountClientModel.update(
                update_values,
                {
                    where: { id: id },
            ...
            if (!(response[0] > 0)) {
                res.status(200).json({
                    validation:
                        "No changes were made to the applied product discount client"
                });
                return;
            }
            res.status(200).json({
                message:
                    "The applied product discount client was updated successfully"
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
                next(error);
