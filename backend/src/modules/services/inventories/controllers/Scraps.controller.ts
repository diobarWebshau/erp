...
class ScrapController {
    static getAll = async (
        ...
        try {
            const response: ScrapModel[] = await
                ScrapModel.findAll();
    ...
    static update = async (
        ...
        try {
            ...
            const responseValidateScrap =
                await ScrapModel.findByPk(id);
            if (!responseValidateScrap) {
                res.status(404).json({
                    validation:
                        "Scrap does not exist"
                });
                return;
            }
            const scrap =
                responseValidateScrap.toJSON();
