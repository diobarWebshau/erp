...
import { RoleModel, PermissionModel, RolePermissionModel }
    ...
    static getById = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!response) {
                res.status(200).json({
                    validation:
                        "No permissions have been assigned to the role"
                });
                return;
            }
    ...
    static create = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (!response) {
                res.status(200).json({
                    validation:
                        "The permission could not be assigned to the role"
                });
                return;
            }
            res.status(200).json({
    ...
    static delete = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (response === 0) {
                res.status(404).json({
                    validation:
                        "No permission assignment found for the specified role"
                });
                return;
            }
            ...
            res.status(200).json({
                message:
                    "Permission successfully deleted from the role."
            });
        } catch (error: unknown) {
            if (error instanceof Error) {
                next(error);
