...
DROP DATABASE IF EXISTS webshau_erp;
CREATE DATABASE webshau_erp;
...
CREATE TABLE inventories(
    id INT AUTO_INCREMENT,
    stock DECIMAL(10,4),
    minimum_stock DECIMAL(10,4) NOT NULL,
    maximum_stock DECIMAL(10,4) NOT NULL,
...
CREATE TABLE product_discounts_clients (
    ...
    discount_percentage DECIMAL(10,4) NOT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY(id),
    FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE,
...
CREATE TABLE purchased_orders_products(
    ...
    recorded_price DECIMAL(10,4) NOT NULL,
    status VARCHAR(100) NOT NULL,
    PRIMARY KEY(id),
...
CREATE TABLE products_inputs(
    ...
    product_id INT,
    input_id INT,
    equivalence DECIMAL(10,4) NOT NULL,
    PRIMARY KEY(id),
    FOREIGN KEY(product_id) REFERENCES products(id) ON DELETE CASCADE,
...
CREATE TABLE productions(
	id INT AUTO_INCREMENT,
    type_order ENUM('internal', 'client') NOT NULL,
    order_id INT,
    product_id INT,
    qty DECIMAL(10,4) NOT NULL,
    scrap DECIMAL(10,4) NOT NULL,
	created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY(id)
);
...
CREATE TABLE applied_product_discounts_ranges (
    ...
    unit_discount DECIMAL(10,4) NOT NULL,
    min_qty DECIMAL(10,4) NOT NULL,
    max_qty DECIMAL(10,4) NOT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY(id),
    FOREIGN KEY (product_discount_range_id) REFERENCES product_discounts_ranges(id) ON DELETE SET NULL,
    FOREIGN KEY (purchase_order_product_id) REFERENCES purchased_orders_products(id) ON DELETE CASCADE
);
...
CREATE TABLE purchased_orders_products_locations_production_lines(
	id INT AUTO_INCREMENT,
    location_production_line_id INT,
    purchase_order_product_id INT,
    PRIMARY KEY(id),
    FOREIGN KEY(location_production_line_id) REFERENCES locations_production_lines(id) ON DELETE SET NULL,
    FOREIGN KEY(purchase_order_product_id) REFERENCES purchased_orders_products(id) ON DELETE CASCADE
);
...
CREATE TABLE internal_production_orders_lines_products(
    ...
    FOREIGN KEY(location_production_line_id) REFERENCES locations_production_lines(id) ON DELETE CASCADE
);
